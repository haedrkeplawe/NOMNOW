...
// ═════════════════════════════════════════════════════════════
//  استبدل دالتي approveSettlement و rejectSettlement
//  في admin_controller.js
// ═════════════════════════════════════════════════════════════
...
exports.approveSettlement = async (req, res) => {
  try {
    ...
    // ── تحديث الأوردرات المرتبطة → withdrawn ─────────────────
...
exports.rejectSettlement = async (req, res) => {
  try {
    ...
    // ── إرجاع الأوردرات → available (الرصيد يعود للمطعم) ──────
    await Order.updateMany(
      {
        settlementId:     settlement._id,
        settlementStatus: "withdrawal_pending",
    ...
    res.status(200).json({
      success: true,
      message: "Settlement rejected — orders returned to available balance",
      settlement,
    });
  } catch (err) {
    console.error(err);
