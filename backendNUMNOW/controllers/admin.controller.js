...
exports.getOrdersMapData = async (req, res) => {
  try {
    const { startDate, endDate, status, country, restaurantId } = req.query;

    const filter = {};

    // فلتر الحالة — "all" أو عدم الإرسال = كل الحالات ما عدا not_confirmed
    // (سلة لم تُرسل بعد، لا فائدة تحليلية من عرضها على الخريطة)
    const rawStatuses = req.query["status[]"] || status;
    if (rawStatuses && rawStatuses !== "all") {
      const statuses = Array.isArray(rawStatuses)
        ? rawStatuses
    ...
    // فلتر الفترة الزمنية — إن لم تُحدد، افتراضياً آخر 30 يوم لتفادي
    // تحميل كامل تاريخ الطلبات دفعة وحدة
    const from = startDate
      ? new Date(startDate)
      : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const to = endDate ? new Date(endDate) : new Date();
    // نضيف نهاية اليوم لتاريخ "to" حتى يشمل طلبات نفس اليوم المُحدد
