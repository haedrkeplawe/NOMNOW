                  ...
                  totalRevenue: {
                    $sum: {
                      $cond: [
                        ...
                        // v4.0 — أساس إيراد المطعم = originalItemsPrice
                        // (fallback على itemsPrice للأوردرات القديمة) —
...
exports.createDriver = async (req, res) => {
  // v2.0 — AB6: خارج الـtry حتى يوصلها catch لو صار فشل بعد الرفع
  // (نفس نمط B6 بتسجيل السائق نفسه بـdriver.controller.js)
  ...
  try {
    ...
    // v2.0 — AB6: كل رفع بيسجّل public_id تبعه بـuploadedPublicIds
    // حتى نقدر نمسحه من Cloudinary لو صار فشل لاحق (مثلاً driver.save())
    const track = (r) => {
    ...
    // v2.0 — AB6: ما نترك صور يتيمة على Cloudinary لو فشل الرفع أو السيف
...
exports.getAllDrivers = async (req, res) => {
  try {
    ...
    const driversWithStats = drivers.map((d) => {
      ...
      // v4.0 — كان حساب يدوي مستقل (متوسط بسيط على طول المصفوفة) بيعطي
      // رقم مختلف عن driver.rating المخزَّن بمجرد ما صار للزبون الواحد
      // أكثر من مدخلة (بعد ربط التقييم بـ orderId). الآن نفس الدالة
      // المشتركة يلي بيستعملها rateDriver.
      const ratings = d.userRatings || [];
      const computedRating =
        ratings.length > 0 ? averageLatestPerUser(ratings) : d.rating || 0;
      return {
        ...d.toObject(),
        totalOrders: s.totalOrders,
...
//   ثم أعد تحميل بيانات السائق ليتجد driverPaymentStatus = "settled" على كل أوردراته
exports.settleDriverEarnings = async (req, res) => {
  try {
    ...
    // v4.0 — الأجرة الحقيقية = originalDeliveryFee (تبقى كاملة حتى لو
    // الطلب فيه عرض توصيل مجاني — الأدمن يتحمّل الفرق، مو السائق)
    const totalEarnings = pendingOrders.reduce(
      (sum, o) => sum + (o.originalDeliveryFee ?? o.deliveryFee ?? 0),
      0,
    );
...
exports.getDriverCashOrders = async (req, res) => {
  try {
    ...
    //          + pendingDeliveryFee و settledDeliveryFee (v4.0: أصبحت تعتمد
    //          على originalDeliveryFee — الأجرة الحقيقية، مش deliveryFee)
    // الألماني: totalPending/totalSettled = مجموع الأجرة الحقيقية (v4.0)
    ...
    const orders = await Order.find({
      ...
      .select(
        "orderNumber totalPrice itemsPrice deliveryFee originalDeliveryFee taxPrice driverPaymentStatus createdAt restaurantId userId items paymentMethod notes deliveryAddress",
      )
    ...
    // v4.0 — أجرة/حصة السائق الحقيقية = originalDeliveryFee (fallback
    // على deliveryFee) — ما بتتأثر بأي عرض توصيل مجاني
    const effectiveFee = (o) => o.originalDeliveryFee ?? o.deliveryFee ?? 0;

    // السوري: يحسب بـ totalPrice — الألماني: يحسب بالأجرة الحقيقية
    const totalPending = isSyrian
      ? pendingOrders.reduce((sum, o) => sum + o.totalPrice, 0)
      : pendingOrders.reduce((sum, o) => sum + effectiveFee(o), 0);

    const totalSettled = isSyrian
      ? settledOrders.reduce((sum, o) => sum + o.totalPrice, 0)
      : settledOrders.reduce((sum, o) => sum + effectiveFee(o), 0);
...
exports.getRestaurantFinancialSummary = async (req, res) => {
  try {
    ...
    // v4.0 — الأساس = originalItemsPrice (قبل خصم أي عرض) — نفس منطق
    // requestSettlement بالضبط، حتى تطابق الأرقام يلي يشوفها الأدمن مع
    // المبلغ الفعلي يلي بينحول للمطعم
    const revBasis = (o) => o.originalItemsPrice ?? o.itemsPrice;
    const totalRevenue = deliveredOrders.reduce((s, o) => s + revBasis(o), 0);
    const totalCommission = totalRevenue * commissionRate;
    const netProfit = totalRevenue - totalCommission;

    const availableToSettle = round(
      availableOrders.reduce(
        (s, o) => s + revBasis(o) * (1 - commissionRate),
        0,
      ),
    );
...
// ─── Coupons (v3.9) ──────────────────────────────────────────
// مقتصر على الطلبات السورية (كاش) بهذه المرحلة — راجع الملاحظات
// بموديل Coupon.js وuser_controller.js (calculateCouponDiscount)
...
exports.getCoupons = async (req, res) => {
  try {
    ...
    const coupons = await Coupon.find(filter)
      .populate("allowedUserIds", "name phone")
      .sort({ createdAt: -1 });
  ...
  try {
    ...
    if (type !== "free_delivery") {
      if (value === undefined || value === null || value === "") {
        return res
          .status(400)
          .json({ message: "value is required for this coupon type" });
      }
      if (type === "percentage" && (value < 1 || value > 100)) {
        return res
          .status(400)
          .json({ message: "Percentage value must be between 1 and 100" });
      }
      if (type === "fixed" && value <= 0) {
        return res
          .status(400)
          .json({ message: "Fixed value must be greater than 0" });
      }
    }
    if (
      ...
      return res.status(400).json({
        message: "allowedUserIds is required when audience is specific_users",
      });
    }
    if (hasExpiry) {
      if (!startDate || !endDate) {
        return res.status(400).json({
          message: "startDate and endDate are required when hasExpiry is true",
        });
      }
      if (new Date(startDate) >= new Date(endDate)) {
        return res
    ...
    const coupon = await Coupon.create({
      ...
      allowedUserIds: audience === "specific_users" ? allowedUserIds : [],
      hasExpiry: !!hasExpiry,
      startDate: hasExpiry ? startDate : null,
...
// PATCH /admin/coupons — بيقبل أي مجموعة من الحقول (تعديل جزئي)،
// بما فيها isActive لتفعيل/تعطيل الكوبون بضغطة وحدة
exports.updateCoupon = async (req, res) => {
  try {
    ...
    allowedFields.forEach((field) => {
      if (updates[field] !== undefined) coupon[field] = updates[field];
    });
