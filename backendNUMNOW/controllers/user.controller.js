...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    if (!deliveryAddress?.fullAddress?.trim() || !hasValidCoordinates) {
      await session.abortTransaction();
      session.endSession();
      ...
      [
        {
          ...
          notes,
          // v2.0 — تحصين 6: لقطة وقت آخر تعديل على السلة لحظة إنشاء
          // الطلب — order:send بيقارنها قبل ما يحذف السلة (نافذة دفع
          // Stripe بمسار DE ممكن تمتد دقايق وتصير إضافة عالسلة بالنص)
