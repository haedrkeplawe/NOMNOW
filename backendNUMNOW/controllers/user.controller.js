...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    const resetUrl = `${process.env.FRONTEND_URL_USER}/reset-password/${resetToken}`;
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #f9f9f9; border-radius: 12px;">
          <div style="text-align: center; margin-bottom: 24px;">
            ...
            <p style="color: #888; font-size: 13px; margin: 4px 0 0;">Customer App</p>
          </div>
...
exports.getUserInfo = async (req, res) => {
  try {
    const user = await User.findById(req.user._id ?? req.user.id).select(
      "name email phone img country gender dateOfBirth addresses createdAt",
    );
    if (!user) {
      return res
        .status(404)
