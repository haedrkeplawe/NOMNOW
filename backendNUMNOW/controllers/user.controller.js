...
const {
  averageLatestPerUser,
  distinctRaterCount,
} = require("../utils/ratingAverage");
const {
...
// نفس فلسفة originalDeliveryFee بالضبط: نحتاجه كأساس ثابت لحساب عمولة/
// أرباح المطعم بمعزل عن أي عرض مطبّق (الأدمن هو من يتحمّل تكلفة العرض).
// item.originalPrice موجود بس لما يكون في عرض discount نشط على الصنف؛
// لو مافي عرض، originalPrice = null فنرجع لـ basePrice نفسه (بلا فرق).
const computeOriginalItemsPrice = (cart) =>
  cart.items.reduce((sum, item) => {
    const extrasTotal = (item.extras || []).reduce((s, e) => s + e.price, 0);
    const unitPrice = item.originalPrice ?? item.basePrice;
    return sum + (unitPrice + extrasTotal) * item.quantity;
  }, 0);
...
exports.getAllRestaurant = async (req, res) => {
  try {
    ...
    // ترتيب ثنائي المحور: المطلوب أولاً، ثم القرب مرجِّحاً عند التعادل.
    // (سابقاً: score مركّب — كان يعطي الترتيب نفسه للفلاتر الثلاثة عند
    //  تساوي التقييمات، لأن الضرب في ثابت موجب لا يغيّر الرتبة)
    const enriched = restaurants.map((r) => ({
      ...r,
      _rating: r.rating || 0,
      _popular: statsMap[r._id.toString()] || 0,
      // بلا موقع تتساوى المسافات فيصير المرجِّح بلا أثر — وهو المطلوب
      _distance: r.distance === null ? Infinity : r.distance,
    }));

    const byNearest = (a, b) => a._distance - b._distance;

    let sorted;
    if (sort === "rating") {
      sorted = enriched.sort(
        (a, b) => b._rating - a._rating || byNearest(a, b),
      );
    } else if (sort === "popular") {
      sorted = enriched.sort(
        (a, b) => b._popular - a._popular || byNearest(a, b),
      );
    } else {
      sorted = enriched.sort(
        (a, b) => byNearest(a, b) || b._rating - a._rating,
      );
    }

    const sortSignal =
      sort === "rating"
        ? enriched.some((r) => r._rating > 0)
          ? "ok"
          : "none"
        : sort === "popular"
          ? enriched.some((r) => r._popular > 0)
            ? "ok"
            : "none"
          : "ok";

    const result = sorted.map(({ _rating, _popular, _distance, ...r }) => r);

    res.status(200).json({
      restaurant: result,
      locationUsed: userCoords !== null,
      sortSignal,
    });
  } catch (err) {
    console.error(err);
...
exports.getAllFood = async (req, res) => {
  try {
    ...
    // ترتيب ثنائي المحور: المطلوب أولاً، ثم القرب مرجِّحاً عند التعادل.
    const enriched = allFoods.map((f) => {
      const distanceKm = distanceMap[f.restaurantId._id.toString()];
      return {
        ...f,
        _rating: f.rating || 0, // ← الحارس المفقود سابقاً (كان f.rating / 5 بلا حارس)
        _popular: statsMap[f._id.toString()] || 0,
        _distance: distanceKm === undefined ? Infinity : distanceKm,
      };
    });
    ...
    const sortSignal =
      sort === "rating"
        ? enriched.some((f) => f._rating > 0)
          ? "ok"
          : "none"
        : sort === "popular"
          ? enriched.some((f) => f._popular > 0)
            ? "ok"
            : "none"
          : "ok";

    const foods = sortedFoods.map(
      ({ _rating, _popular, _distance, ...f }) => f,
    );
...
async function updateRestaurantRating(restaurantId) {
  ...
  // v4.0 — نجمع كل التقييمات من كل الأكلات، ثم نمرّرها كاملة للدالة
  // المشتركة (كانت هون قبل توحيدها بـ utils/ratingAverage، بتعتمد الآن
  // updatedAt ?? createdAt بدل createdAt وحده — راجع القسم 8-أ بملف المهمة)
  const allRatings = foods.flatMap((food) => food.userRatings);
  if (!allRatings.length) return;

  const avg = averageLatestPerUser(allRatings);
...
exports.rateFood = async (req, res) => {
  ...
  try {
    ...
    // v4.0 — متوسط الأكلة عبر الدالة المشتركة (آخر تقييم لكل مستخدم،
    // بحسب updatedAt ?? createdAt)
    food.rating = averageLatestPerUser(food.userRatings);
    await food.save();
    ...
    res.status(200).json({
      ...
      rating: food.rating,
      totalRatings: distinctRaterCount(food.userRatings), // v4.0 — عدد مقيّمين مميّزين لا عدد المدخلات
      myRating: {
...
exports.rateDriver = async (req, res) => {
  ...
  try {
    ...
    // v4.0 — مطابقة بـ userId + orderId معاً (نفس شكل rateOrder)، بدل
    // userId وحده يلي كان يخلي تقييم الطلب الثاني يستبدل تقييم الأول
    ...
    // v4.0 — متوسط السائق عبر الدالة المشتركة (آخر تقييم لكل مستخدم)
    driver.rating = averageLatestPerUser(driver.userRatings);

    await driver.save();

    // v4.0 — نقرأ myRating بنفس المطابقة المزدوجة، وإلا رجعت الاستجابة
    // تقييم طلب سابق للسائق نفسه بدل الطلب الحالي
    const myRating =
      existingRating ||
      driver.userRatings.find(
        (r) =>
          r.userId.toString() === userId.toString() &&
          r.orderId?.toString() === order._id.toString(),
      );
    ...
    res.status(200).json({
      ...
      totalRatings: distinctRaterCount(driver.userRatings), // v4.0 — عدد مقيّمين مميّزين لا عدد التوصيلات المُقيَّمة
      myRating: {
        rating: myRating.rating,
        comment: myRating.comment || null,
      },
...
exports.rateOrder = async (req, res) => {
  try {
    ...
    for (const foodId of foodIds) {
      ...
      // v4.0 — متوسط الأكلة عبر الدالة المشتركة (آخر تقييم لكل مستخدم،
      // بحسب updatedAt ?? createdAt — راجع القسم 8-أ بملف المهمة)
      food.rating = averageLatestPerUser(food.userRatings);
      await food.save();
    }
...
// v2.0 — تحصين 7: country مطلوب الآن — Promotion.country يعزل العروض
// بين السوقين (SY/DE) أو "ALL" للعروض المشتركة، وكان مُتجاهَلاً بالكامل
async function getActivePromotionsForFood(foodId, country) {
  const now = new Date();
...
exports.addToCart = async (req, res) => {
  ...
  try {
    ...
    // v2.0 — تحصين 3: نحوّل الكمية لرقم قبل أي فحص أو استخدام — كانت
    // "2" (نص) تعبر الفحص الثلاثي وتتحول جمع نصوص بمسار الدمج (2+"2"="22")
    const qty = Number(quantity);
    if (!quantity || qty < 1 || !Number.isInteger(qty)) {
      return res.status(400).json({ message: m.cart.invalidQuantity });
    }

    // v2.0 — تحصين 3: foodId غير صالح كان يرمي CastError يوصل 500 بدل 400
    // (نفس نمط rateFood المستخدم أصلاً على نفس الحقل)
    if (!mongoose.Types.ObjectId.isValid(foodId)) {
    ...
    // v2.0 — تحصين 8: فحص حالة المطعم مبكراً (نفس فحص order:send) —
    // وبنفس النداء منجيب country المطعم لفلترة العروض بتحصين 7
    const restaurant = await Restaurant.findById(food.restaurantId).select(
      "status country",
    );
    ...
    if (cart.restaurantId.toString() !== food.restaurantId.toString()) {
      // v2.0 — تحصين 4: replaceCart:true يستبدل السلة بمطعم جديد بدل
      // رفضها — نداء واحد ذرّي بدل حذف العناصر واحداً واحداً من الفرونت
      if (!replaceCart) {
        return res.status(400).json({
    ...
    // v2.0 — تحصين 7: نمرر country المطعم حتى ما يظهر عرض خاص ببلد
    // على مطعم/مستخدم من بلد تاني (عرض SY ما يطبّق على مطعم DE)
    const activePromotions = await getActivePromotionsForFood(
      foodId,
      restaurant.country,
    );
    ...
    // v2.0 — تحصين 1: السعر يُستنبط من food.extras لا من الطلب — العميل
    // كان يقرر سعر الإضافة بنفسه (حتى 0 لإضافة مدفوعة أصلاً)
    const selectedExtras = [];
    for (const e of extras) {
      const found = food.extras.find((x) => x.name === e?.name);
      if (!found) {
        return res.status(400).json({ message: m.cart.invalidExtra });
      }
      selectedExtras.push({ name: found.name, price: found.price });
    }
    const extrasTotal = selectedExtras.reduce((s, x) => s + x.price, 0);
    const totalItemPrice = (effectiveBasePrice + extrasTotal) * qty;
    ...
    const existingItem = cart.items.find(
      (item) =>
        ...
        areExtrasEqual(item.extras, selectedExtras),
    );
...
// v2.0 — تحصين 5: تعديل كمية عنصر بمكانه — بلا حذف/إعادة إنشاء الوثيقة،
// فما تضيع couponCode/hasFreeDelivery/freeDeliveryPromotionId (حقول
// على وثيقة السلة لا على العنصر) زي ما كان صاير بالحل التعويضي القديم
// (DELETE ثم POST) بالفرونت
exports.updateCartItemQuantity = async (req, res) => {
  ...
  try {
    ...
    // basePrice مخزَّن أصلاً بعد أي خصم عرض (effectiveBasePrice وقت
    // الإضافة) — نفس معادلة addToCart بالضبط، بس بكمية جديدة
    const extrasTotal = item.extras.reduce((s, e) => s + e.price, 0);
    item.quantity = qty;
    item.totalItemPrice = parseFloat(
      ((item.basePrice + extrasTotal) * qty).toFixed(2),
    );

    // إعادة حساب مجموع السلة من الصفر (أدق من طرح/جمع تراكمي)
    cart.totalCartPrice = cart.items.reduce(
...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    // v2.0 — تحصين 2: نرفض أي عنوان ناقص أو إحداثيات غير صالحة قبل ما
    // نلمس السلة أو ننشئ أي شي — الباك يرفض بغض النظر عن سلوك أي عميل
    const coordinates = deliveryAddress?.location?.coordinates;
    ...
    // v2.0 — تحصين 2 (تشديد إضافي): "، ، ، " ونحوها تعبر .trim() لأنها
    // ليست فراغاً بحتاً، لكنها بلا أي محتوى فعلي — نطرح الفواصل والمسافات
    // أولاً ثم نطلب حداً أدنى من المحتوى الحقيقي
    const addressCore = (deliveryAddress?.fullAddress || "").replace(
      /[,\s]/g,
      "",
    );
    const hasValidAddress = addressCore.length >= 3;
    ...
    if (!hasValidAddress || !hasValidCoordinates) {
      ...
      return res
        .status(400)
        .json({ message: m.order.invalidDeliveryAddress });
    }
    ...
    // v4.0 — سعر الأصناف الأصلي قبل خصم عروض discount + قيمة الخصم
    // الناتج عنها (يتحمّله الأدمن بالكامل — راجع Order.js)
    const originalItemsPrice = Number(
      computeOriginalItemsPrice(cart).toFixed(2),
    );
    const promotionDiscount = Number(
      (originalItemsPrice - itemsPrice).toFixed(2),
    );
    ...
    // v2.0 — ب2: هذا الحفظ لازم يصير هون، قبل التقاط cartSnapshotAt وقبل
    // Order.create — لأنو save() بيبدّل cart.updatedAt (timestamps: true).
    // لو ضلّ بعد Order.create متل قبل، كل طلب فيه كوبون انشال بصمت
    // (وصل حد الاستخدام مثلاً) كان بيولّد لقطة أقدم من السلة الفعلية،
    // فـ order:send كان دايماً يعتبرها "متغيّرة" حتى لو المستخدم ما لمسها.
      ...
      [
        {
          ...
          notes,
          // v2.0 — تحصين 6 (مُصحَّحة بب2): لقطة cart.updatedAt لحظة إنشاء
          // الطلب — تُلتقط هون بعد أي حفظ ممكن يصير على السلة بهاي
          // الدالة (راجع كتلة couponResult.removed أعلاه)، فتعكس آخر
          // حالة فعلية للسلة. order:send بيقارنها قبل ما يحذف السلة
          // (نافذة دفع Stripe بمسار DE ممكن تمتد دقايق وتصير إضافة
          // عالسلة بالنص)
          cartSnapshotAt: cart.updatedAt,
        },
      ],
      { session },
    );
    ...
    delete responseOrder.driverSearchAttempt;
    // v2.0 — تحصين 6: cartSnapshotAt حقل داخلي بحت (راجع order:send)
    delete responseOrder.cartSnapshotAt;
    if (isGerman) {
