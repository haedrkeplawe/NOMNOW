...
exports.deleteAddress = async (req, res) => {
  try {
    ...
    if (user.addresses.length === 1) {
      return res
        .status(400)
        .json({ message: "Cannot delete your only address" });
    }
...
exports.addToCart = async (req, res) => {
  try {
    ...
    if (food.sizes && food.sizes.length > 0) {
      // الطعام عنده أحجام → لازم يختار حجم
      if (!size || !size.name) {
        return res
          .status(400)
          .json({ message: "This item requires a size selection" });
      }
      ...
      if (size && size.name) {
        return res
          .status(400)
          .json({ message: "This item does not have sizes" });
      }
    }
...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    if (restaurant.status !== "open") {
      ...
      return res
        .status(400)
        .json({ message: "Restaurant is currently closed" });
    }
