...
// AUTH
// v4.3 — إعادة تعيين كلمة المرور بنظام كود (OTP) عبر SMS حصراً (المستخدم
// أصلاً ما عنده حقل إيميل بالسكيما). بنفس نمط forgotPassword/resetPassword
// عند المطعم والسائق (v4.2/v4.3)، بدل الـ503 المعطّل سابقاً.
exports.forgotPassword = async (req, res) => {
  try {
    const m = getMessages(req).user;
    const { phone } = req.body;

    if (!phone) return res.status(400).json({ message: m.auth.missingFields });

    const user = await User.findOne({ phone });
    if (!user) return res.status(404).json({ message: m.auth.phoneNotFound });

    if (user.status === "blocked")
      return res.status(403).json({ message: m.auth.accountBlocked });

    // v4.3 — نفس أداة الـ throttle الموحّدة المستخدمة بتسجيل الدخول، لكن
    // على حقل passwordResetOtpExpire المستقل (مش phoneOtp)
    if (!canSendOtp(user.passwordResetOtpExpire)) {
      return res.status(429).json({
        message: m.auth.resendTooSoon,
        retryAfterSeconds: secondsUntilNextOtp(user.passwordResetOtpExpire),
      });
    }

    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    user.passwordResetOtp = otp;
    user.passwordResetOtpExpire = Date.now() + 60 * 60 * 1000;
    await user.save();
    ...
    res.status(200).json({
      message: m.auth.otpSent + ` Your OTP: ${otp}`,
      requiresReset: true,
    });
  } catch (err) {
    console.error(err);
...
// v4.3 — تحقق الكود + تعيين كلمة المرور الجديدة بخطوة واحدة
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (newPassword !== confirmPassword) {
      return res.status(400).json({ message: m.auth.passwordMismatch });
    }
    if (newPassword.length < 6) {
      return res.status(400).json({ message: m.auth.passwordTooShort });
    }
    ...
    // ملاحظة: توكن المستخدم JWT عديم الحالة (stateless) وغير مخزَّن بقاعدة
    // البيانات، فما في refreshToken نلغيه هون متل ما بصير بالمطعم — أي
    // جلسة قديمة رح تضل صالحة لحد ما تنتهي صلاحية التوكن نفسه (exp)
...
exports.register = async (req, res) => {
  const m = getMessages(req).user;
  try {
    // v4.1 — حذف dateOfBirth نهائياً من التسجيل (لم يعد حقلاً مطلوباً ولا مخزَّناً)
...
exports.loginWithPhone = async (req, res) => {
  try {
    ...
    if (!user.isVerifiedPhone) {
      ...
      // (بالإضافة لـ authLimiter العام بـ server.js — هاد خاص بنفس الحساب)
      if (!canSendOtp(user.phoneOtpExpire)) {
        return res.status(429).json({
          message: m.auth.resendTooSoon,
          retryAfterSeconds: secondsUntilNextOtp(user.phoneOtpExpire),
        });
      }

      const otp = Math.floor(100000 + Math.random() * 900000).toString();

      // v2.0 — إرسال SMS حقيقي عبر Aman Gate. بمرحلة الاختبار الحالية (بناءً على
      ...
      return res.status(200).json({
        ...
        otp, // بس للتجربة — بالإبقاء عليه مؤقتاً بطلب صريح لمرحلة الاختبار
      });
    }
...
exports.updateProfile = async (req, res) => {
  ...
  try {
    ...
    // v4.1 — حذف dateOfBirth نهائياً من تعديل البروفايل
    ...
    const updatedUser = await User.findByIdAndUpdate(
      ...
      { $set: updateData },
      { new: true },
    ).select("name phone img country gender");
...
exports.getUserInfo = async (req, res) => {
  try {
    const user = await User.findById(req.user._id ?? req.user.id).select(
      "name phone img country gender addresses createdAt",
    );
    if (!user) {
      return res
        .status(404)
...
exports.addToCart = async (req, res) => {
  ...
  try {
    ...
    const {
      ...
      extras = [],
      size = null,
      // v4.2 — تحصين 4: علم اختياري لتبديل السلة لمطعم آخر بنداء واحد
      replaceCart = false,
    } = req.body;
    // v4.2 — تحصين 2: تحويل صريح لرقم أول الدالة، بدل ما تُستخدم الكمية
    // الخام (ممكن توصل كنص) بعمليات حسابية/دمج لاحقة
    ...
    // v4.2 — تحصين 3: foodId لازم يكون ObjectId صالح قبل أي استعلام،
    // وإلا Mongoose يرمي CastError توصل للمستخدم كـ500 بدل 400 نظيف
    if (!mongoose.Types.ObjectId.isValid(foodId)) {
    ...
    // v4.2 — تحصين 1 (الثغرة الأخطر): مطابقة كل إضافة مع food.extras
    // بالاسم — لا نثق أبداً بسعر جايي من العميل مباشرة بالـ body
    // (كان ممكن يرسل price: 0 لإضافة مدفوعة وتمرّ كاملة)
    ...
    if (cart.restaurantId.toString() !== food.restaurantId.toString()) {
      ...
      // v4.2 — تحصين 4: replaceCart:true → تصفير السلة وربطها بالمطعم
      // الجديد بالذاكرة فقط؛ الحفظ الفعلي بنداء cart.save() الوحيد بآخر
      // الدالة (نفس الترانزاكشن المنطقي، بلا نداء save منفصل)
      cart.items = [];
    ...
    // 7️⃣ حساب سعر الإضافات — من selectedExtras المُتحقَّق منها لا الخام
...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    // v4.2 — تحصين 1: التحقق من صحة العنوان/الإحداثيات قبل لمس السلة
    // أو أي عملية أخرى — وإلا الطلب يفشل لاحقاً عند طبقة المخطط
    // (ValidationError → 500) بدل رفض نظيف من هون (400)
    const cleanFullAddress = String(deliveryAddress?.fullAddress ?? "").replace(
      /[\s,]/g,
      "",
    );
    ...
    if (cleanFullAddress.length < 3 || !isValidCoords) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json({ message: m.order.invalidDeliveryAddress });
    }
    ...
    // وبعدين هاد الحفظ يحدّثه لقيمة أجدد، فيظنّ order:send (بالسوكيت)
    // إنه السلة تغيّرت فعلياً بكل طلب فيه كوبون انبطل هون
      ...
      [
        {
          ...
          couponDiscount,
          deliveryAddress,
          // v4.2 — تحصين 6: لقطة updatedAt تبع السلة لحظة إنشاء الطلب
          // (بعد أي حفظ محتمل أعلاه) — order:send بيقارنها قبل حذف السلة
