...
// update
/* 
أُضيف size = null للـ destructuring من الـ request body
أُضيف منطق كامل للتحقق من الحجم — إذا الوجبة عندها sizes والزبون ما أرسل size يرجع خطأ 400
السعر الأساسي basePrice صار يأخذ من الحجم إذا موجود، وإلا من food.price كما كان
شرط المقارنة لـ existingItem أُضيف له (item.size?.name ?? null) === (selectedSize?.name ?? null) — يعني نفس الوجبة بحجمين مختلفين تُعامَل كعنصرين منفصلين في السلة
حقل size: selectedSize أُضيف عند push العنصر للسلة
*/
exports.addToCart = async (req, res) => {
  try {
    ...
    // 2️⃣ التحقق من الحجم إذا الوجبة عندها أحجام
...
// update
// سطر واحد فقط — أُضيف size: item.size ?? null في الـ mapping عند تحويل عناصر السلة لأوردر
