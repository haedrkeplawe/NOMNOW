...
exports.updateDriverInfo = async (req, res) => {
  try {
    ...
    const ok1 = await updateDocument("id", "idImage");
    if (!ok1) return;
    const ok2 = await updateDocument("driving_license", "drivingLicenseImage");
    if (!ok2) return;
    const ok3 = await updateDocument(
...
exports.getOrdersHistory = async (req, res) => {
  try {
    ...
    // ── إجمالي عدد الطلبات (بدون فلتر) ──────────────────
    const totalOrdersCount = await Order.countDocuments({
      driverId,
      orderStatus: { $in: ["delivered", "cancelled"] },
    });

    // ── إحصائيات اليوم (دائماً) ──────────────────────────
