...
exports.loginWithPhone = async (req, res) => {
  try {
    ...
    if (!driver.isVerifiedPhone) {
      ...
      if (!canSendOtp(driver.phoneOtpExpire)) {
        return res.status(429).json({
          message: m.auth.resendTooSoon,
          retryAfterSeconds: secondsUntilNextOtp(driver.phoneOtpExpire),
        });
      }
...
// v4.3 — إعادة تعيين كلمة المرور بنظام كود (OTP) عبر SMS حصراً (رقم الهاتف
// فقط، بدون خيار إيميل رغم توفره بالسكيما)، بدل رابط الإيميل القديم.
// بنفس نمط forgotPassword/resetPassword عند المطعم (v4.2).
exports.forgotPassword = async (req, res) => {
  try {
    ...
    if (!canSendOtp(driver.passwordResetOtpExpire)) {
      return res.status(429).json({
        message: m.auth.resendTooSoon,
        retryAfterSeconds: secondsUntilNextOtp(driver.passwordResetOtpExpire),
      });
    }

    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    driver.passwordResetOtp = otp;
    driver.passwordResetOtpExpire = Date.now() + 60 * 60 * 1000;
    await driver.save();
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (!driver.passwordResetOtp || driver.passwordResetOtp !== otp) {
      return res.status(400).json({ message: m.auth.invalidOtp });
    }
    if (
      !driver.passwordResetOtpExpire ||
      driver.passwordResetOtpExpire < Date.now()
    ) {
      return res.status(400).json({ message: m.auth.otpExpired });
    }
    ...
    driver.passwordResetOtpExpire = undefined;
    // ملاحظة: توكن السائق JWT عديم الحالة (stateless) وغير مخزَّن بقاعدة
