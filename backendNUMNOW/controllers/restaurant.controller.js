...
// v4.2 — إعادة تعيين كلمة المرور بنظام كود (OTP) عبر SMS أو إيميل، بدل
// رابط الإيميل القديم. "type" يحدد القناة (phone/email)، بنفس نمط resendOtp.
exports.forgotPassword = async (req, res) => {
  try {
    ...
    if (!type || !["phone", "email"].includes(type)) {
      return res.status(400).json({ message: m.auth.invalidResetType });
    }
    ...
    if (type === "phone") {
      ...
      user = await RestaurantUser.findOne({ phone });
      if (!user)
    ...
    // v4.2 — نفس أداة الـ throttle الموحّدة المستخدمة بتسجيل الدخول، لكن
    // على حقل passwordResetOtpExpire المستقل (مش phoneOtp/emailOtp)
    ...
    // v4.2 — مرحلة اختبار: فشل الإرسال ما بيوقف الطلب (متل باقي مسارات OTP
    // الحالية)، والكود منرجعه صراحة بالـ response مؤقتاً لأغراض التطوير —
    ...
    if (type === "phone") {
      ...
      if (!sent) {
        console.warn(
          `⚠️ Password reset email failed for ${user.email} — continuing (test mode)`,
        );
      }
    }

    const baseMessage =
      type === "phone" ? m.auth.otpSentPhone : m.auth.otpSentEmail;

    res.status(200).json({
      message: baseMessage + ` Your OTP: ${otp}`,
      requiresReset: true,
    });
  } catch (err) {
    console.error(err);
...
// v4.2 — تحقق الكود + تعيين كلمة المرور الجديدة بخطوة واحدة (بدل خطوتين
// منفصلتين "تحقق" ثم "تعيين" كما كان مخطط له بالرابط القديم)
exports.resetPassword = async (req, res) => {
  try {
    const m = getMessages(req);
    const { type, phone, email, otp, newPassword, confirmPassword } = req.body;
...
exports.resendOtp = async (req, res) => {
  try {
    ...
    if (type === "phone") {
      ...
      // v2.0 — نفس الأداة الموحّدة المستخدمة بـ loginWithPhone (بدل تكرار
      // نفس حساب الفارق الزمني هون بشكل منفصل)
      if (!canSendOtp(user.phoneOtpExpire)) {
        return res.status(429).json({
          message: m.auth.resendTooSoon,
          retryAfterSeconds: secondsUntilNextOtp(user.phoneOtpExpire),
...
exports.createFood = async (req, res) => {
  try {
    ...
    // v3.7 — التحقق من القسم العام لو أُرسل (اختياري)
    if (mainCategoryId) {
      const mainCategory = await MainCategory.findById(mainCategoryId);
      if (!mainCategory) {
        return res.status(400).json({ message: m.food.invalidMainCategory });
      }
    }
    ...
    const foodData = new Food({
      restaurantId: req.user.restaurantId,
      categoryId,
      mainCategoryId: mainCategoryId || null, // v3.7
      name,
      description,
    ...
    const foodWithCategory = await Food.findById(foodData._id)
      .populate("categoryId")
      .populate("mainCategoryId"); // v3.7
    res
      .status(201)
...
exports.updateFood = async (req, res) => {
  try {
    ...
    // v3.7 — تحديث القسم العام لو أُرسل
    if (mainCategoryId !== undefined) {
      if (mainCategoryId === null || mainCategoryId === "") {
        food.mainCategoryId = null;
      } else {
        const mainCategory = await MainCategory.findById(mainCategoryId);
        if (!mainCategory) {
          return res.status(400).json({ message: m.food.invalidMainCategory });
        }
        food.mainCategoryId = mainCategoryId;
      }
    }
    ...
    const foodWithCategory = await Food.findById(food._id)
      .populate("categoryId")
      .populate("mainCategoryId"); // v3.7
...
exports.getOrders = async (req, res) => {
  try {
    ...
    // v4.0 — أساس عمولة/صافي ربح المطعم = originalItemsPrice (سعر الأصناف
    // قبل خصم أي عرض) — حتى لا يتأثر المطعم بعروض الخصم؛ الأدمن هو من
    // يتحمّل الفرق. fallback على itemsPrice للأوردرات القديمة قبل هالتحديث.
    const enrichedOrders = orders.map((o) => {
      const commissionBasis = o.originalItemsPrice ?? o.itemsPrice;
      return {
        ...o.toObject(),
        commission:
          o.orderStatus === "delivered"
            ? Math.round(commissionBasis * (commissionRate / 100) * 100) / 100
            : null,
        netProfit:
          o.orderStatus === "delivered"
            ? Math.round(commissionBasis * (1 - commissionRate / 100) * 100) /
              100
            : null,
      };
    });
    ...
    pipeline.push({
      $facet: {
        data: [
          ...
          { $limit: limit },
          {
            // v4.0 — أساس العمولة/صافي الربح = originalItemsPrice (fallback
            // على itemsPrice للأوردرات القديمة) — المطعم ما يتأثر بعروض الخصم
            $addFields: {
              _commissionBasis: {
                $ifNull: ["$originalItemsPrice", "$itemsPrice"],
              },
            },
          },
          {
...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    // ── helper: إحصائيات ──────────────────────────────────────
    // v4.0 — مبيعات المطعم = originalItemsPrice (قبل خصم أي عرض) —
    // المطعم ما يتأثر بعروض الخصم؛ الأدمن يتحمّل الفرق
    const revBasis = (o) => o.originalItemsPrice ?? o.itemsPrice;
    const calcStats = (orders, allOrders) => {
      const totalSales = orders.reduce((s, o) => s + revBasis(o), 0);
    ...
    todayOrders.forEach((o) => {
      ...
      salesByHour[h].sales += revBasis(o);
...
exports.getFinancialOverview = async (req, res) => {
  try {
    ...
    const [
      ...
      }).select("itemsPrice originalItemsPrice"),
    ...
    // v4.0 — الأساس = originalItemsPrice (قبل خصم أي عرض) — المطعم ما
    // يتحمّل تكلفة عروض الخصم؛ الأدمن هو من يتحمّلها بالكامل
    const revBasis = (o) => o.originalItemsPrice ?? o.itemsPrice;
    const calcNet = (orders) =>
      orders.reduce((s, o) => s + revBasis(o) * (1 - commissionRate), 0);
    const calcRev = (orders) => orders.reduce((s, o) => s + revBasis(o), 0);
...
exports.requestSettlement = async (req, res) => {
  ...
  try {
    ...
    }).select("itemsPrice originalItemsPrice _id");

    if (availableOrders.length === 0) {
      return res.status(400).json({ message: m.settlement.noAvailableBalance });
    }

    // v4.0 — أساس مبلغ التسوية الفعلي = originalItemsPrice (قبل خصم أي
    // عرض) — المطعم لازم ياخد ربحه كامل بغض النظر عن عروض الخصم؛ الأدمن
    // هو من يتحمّل الفرق من عمولته. fallback لـ itemsPrice للأوردرات
    // القديمة (قبل هالتحديث) يلي ما عندها originalItemsPrice محفوظ.
    const totalAmount = round(
      availableOrders.reduce(
        (s, o) =>
          s + (o.originalItemsPrice ?? o.itemsPrice) * (1 - commissionRate),
        0,
      ),
    );
...
exports.getSalesReports = async (req, res) => {
  try {
    ...
    const [currentOrders, prevOrders, allPeriodOrders] = await Promise.all([
      ...
      }).select("itemsPrice originalItemsPrice createdAt"),
    ...
    // v4.0 — الأساس = originalItemsPrice (قبل خصم أي عرض) — إيرادات
    // المطعم وربحه ما يتأثروا بعروض الخصم؛ الأدمن يتحمّل الفرق
    const revBasis = (o) => o.originalItemsPrice ?? o.itemsPrice;
    // ── helper: حساب إجماليات ─────────────────────────────────
    const calcSummary = (orders) => {
      const totalRevenue = orders.reduce((s, o) => s + revBasis(o), 0);
      const totalOrders = orders.length;
