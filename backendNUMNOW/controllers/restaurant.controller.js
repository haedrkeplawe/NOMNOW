...
// ─────────────────────────────────────────────────────────────
//  أضف هذا السطر في أعلى restaurant_controller.js مع باقي الـ imports
// ─────────────────────────────────────────────────────────────
// const Settlement = require("../models/Settlement");

// ─────────────────────────────────────────────────────────────
//  استبدل دالة getFinancialOverview الحالية بهذه
// ─────────────────────────────────────────────────────────────
...
exports.getFinancialOverview = async (req, res) => {
  try {
    ...
    // ── جلب الأوردرات والسحوبات بالتوازي ───────────────────────
    const [deliveredOrders, cancelledOrders, settlements] = await Promise.all([
      Order.find({ restaurantId, orderStatus: "delivered" }).select(
        "itemsPrice createdAt",
      ),
      Order.find({ restaurantId, orderStatus: "cancelled" }).select(
        "itemsPrice createdAt",
      ),
      Settlement.find({ restaurantId }).select("amount status createdAt"),
    ]);

    // ── حسابات الإيرادات ─────────────────────────────────────────
    const totalRevenue = deliveredOrders.reduce(
      (sum, o) => sum + o.itemsPrice,
      0,
    );
    const totalCommission = totalRevenue * commissionRate;
    const netProfit = totalRevenue - totalCommission;

    // ── Available vs Pending Settlement (بدون السحوبات) ──────────
    let grossAvailable = 0; // أرباح قديمة (مؤهلة للسحب بالوقت)
    let pendingSettlement = 0; // أرباح لسه في فترة الانتظار

    deliveredOrders.forEach((o) => {
      const orderNetProfit = o.itemsPrice * (1 - commissionRate);
      if (new Date(o.createdAt) <= settlementCutoff) {
        grossAvailable += orderNetProfit;
      } else {
        pendingSettlement += orderNetProfit;
      }
    });

    // ── حساب السحوبات ────────────────────────────────────────────
    let totalWithdrawn = 0; // مجموع السحوبات المعتمدة
    let pendingWithdrawal = 0; // طلبات سحب بانتظار موافقة الأدمن
    ...
    // ── الرصيد المتاح للسحب الآن (بعد خصم ما سُحب وما في انتظار الموافقة) ──
    const availableToSettle = Math.max(
      0,
      grossAvailable - totalWithdrawn - pendingWithdrawal,
    );
    ...
    res.status(200).json({
      ...
      overview: {
        ...
        grossAvailable: round(grossAvailable), // أرباح مؤهلة بالوقت
        availableToSettle: round(availableToSettle), // متاح للسحب الآن
        pendingSettlement: round(pendingSettlement), // لسه في فترة الانتظار
        totalWithdrawn: round(totalWithdrawn), // تم سحبه فعلاً
        pendingWithdrawal: round(pendingWithdrawal), // طلب سحب قيد الانتظار
        cancelledLoss: round(cancelledLoss),
      },
      counts: {
        deliveredOrders: deliveredOrders.length,
        cancelledOrders: cancelledOrders.length,
        totalSettlements: settlements.filter((s) => s.status === "approved")
          .length,
        pendingRequests: settlements.filter((s) => s.status === "pending")
          .length,
      },
    });
  } catch (err) {
...
exports.requestSettlement = async (req, res) => {
  try {
    ...
    settlements.forEach((s) => {
      ...
      else if (s.status === "pending") pendingWithdrawal += s.amount;
    });
    ...
    if (Number(amount) > availableToSettle) {
      return res.status(400).json({
        message: `Requested amount exceeds available balance. Available: ${availableToSettle} ${restaurant.currency || "SYP"}`,
        availableToSettle,
      });
    }
...
// ─────────────────────────────────────────────────────────────
//  دالة جديدة: المطعم يشوف سجل سحوباته
// ─────────────────────────────────────────────────────────────
...
exports.requestSettlement = async (req, res) => {
  try {
    ...
    if (Number(amount) > availableToSettle) {
      return res.status(400).json({
        message: `Requested amount exceeds available balance. Available: ${availableToSettle} ${restaurant.currency || "SAR"}`,
        availableToSettle,
      });
    }
