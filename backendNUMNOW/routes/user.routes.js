...
// v4.3 — بدون :token بالـURL، الكود (otp) صار يترسل بالـbody
...
router
  ...
  // v2.0 — تحصين 5: PATCH لتعديل كمية عنصر بمكانه (بدل DELETE ثم POST)
  .patch(auth, userController.updateCartItemQuantity)
  .delete(auth, userController.removeFoodFromCart);

// v3.9 — coupons
router.post("/cart/coupon", auth, userController.applyCoupon);
router.delete("/cart/coupon", auth, userController.removeCoupon);
router.get("/coupons", auth, userController.getMyCoupons);
...
// الأقسام العامة (يديرها الأدمن، قراءة فقط)
router.get("/main-categories", auth, userController.getMainCategories);

// Fcm Token update - لإرسال إشعارات push للمستخدم عن تحديثات طلبه
router.patch("/fcm-token", auth, userController.updateFcmToken);
