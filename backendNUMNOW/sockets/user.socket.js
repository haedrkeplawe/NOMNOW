  ...
  userNS.on("connection", (socket) => {
    ...
    socket.on("order:send", async (data) => {
      try {
        ...
        if (cart) {
          // v2.0 — تحصين 6: لو السلة تغيّرت من وقت إنشاء الطلب (مثلاً
          // صنف انضاف بنافذة دفع Stripe يلي ممكن تمتد دقايق بمسار DE)
          // ما نحذفها بصمت — نرجّع الطلب not_confirmed ونخلي المستخدم
          // يراجع سلته بدل ما يضيع الصنف بلا أي إشعار
          if (
