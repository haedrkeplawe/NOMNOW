...
async function autoConfirmOrder(order, io) {
  ...
  if (freshDriver.country === "SY") {
    updatedOrder.driverPaymentStatus = "pending";
    await Driver.findByIdAndUpdate(driverId, {
      $inc: { cashCollected: updatedOrder.totalPrice },
      availability: "online",
    });
  } else {
    updatedOrder.driverPaymentStatus = "not_applicable";
    await Driver.findByIdAndUpdate(driverId, { availability: "online" });
  }
      ...
      try {
        ...
        const [order, driver] = await Promise.all([
          Order.findById(orderId).select(
            "driverId restaurantId totalPrice orderNumber",
          ),
