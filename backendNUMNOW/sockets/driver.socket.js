    ...
    socket.on("order:driverResponse", async (data) => {
      try {
        ...
        const driver = await Driver.findById(driverId).select(
          "country cashCreditLimit cashCollected",
        );
        ...
        // new
        // ✅ تحقق من حد الكاش للسائق السوري قبل القبول
        ...
        setTimeout(
          async () => {
            try {
              ...
              const freshDriver = await Driver.findById(driverId).select(
                "country cashCollected",
              );
              ...
              // ── السائق السوري: يستلم كاش → pending + زيادة cashCollected
              if (freshDriver.country === "SY") {
                freshOrder.driverPaymentStatus = "pending";
                await Driver.findByIdAndUpdate(driverId, {
                  $inc: { cashCollected: freshOrder.totalPrice },
                  availability: "online",
                });
              } else {
                // ── السائق الألماني: لا يلمس كاش → not_applicable
                freshOrder.driverPaymentStatus = "not_applicable";
                await Driver.findByIdAndUpdate(driverId, {
                  availability: "online",
                });
              }
