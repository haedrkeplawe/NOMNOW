const translations = {
  en: {
    // ── Auth ──────────────────────────────────────────────────
    auth: {
      userNotFound: "User not found",
      phoneNotRegistered: "Phone not registered",
      emailNotRegistered: "Email not registered",
      invalidPassword: "Invalid password",
      invalidOtp: "Invalid OTP",
      otpExpired: "OTP expired",
      phoneNotFound: "Phone not found",
      emailNotFound: "Email not found",
      invalidOrExpiredToken: "Invalid or expired token",
      noRefreshToken: "No refresh token",
      invalidRefreshToken: "Invalid refresh token",
      refreshTokenExpired: "Refresh token expired",
      missingFields: "Missing required fields",
      otpSentPhone: "OTP sent successfully",
      otpSentEmail: "OTP sent to your email",
      passwordResetSuccess: "Password reset successful",
      resetLinkSent: "Reset link sent to email",
      resetEmailFailed: "Failed to send reset email. Please try again.",
      loggedOut: "Logged out",
      // v2.0 — SMS OTP (Aman Gate) real sending
      otpSendFailed: "Failed to send verification code. Please try again.",
      resendTooSoon: "Please wait a bit before requesting another code",
      resendSuccess: "OTP resent successfully",
      resendFailed: "Failed to resend OTP. Please try again.",
      invalidResendType: "Invalid resend type",
      // v4.2 — إعادة تعيين كلمة المرور بنظام الكود
      invalidResetType: "Type must be 'phone' or 'email'",
      passwordMismatch: "Passwords do not match",
      passwordTooShort: "Password must be at least 6 characters",
      // v4.8 — restaurant account blocked by admin
      accountBlocked: "Your account has been blocked",
    },

    // ── Category ──────────────────────────────────────────────
    category: {
      nameRequired: "Category name is required",
      alreadyExists: "Category already exists",
      notFound: "Category not found",
      created: "Category created",
      updated: "Category updated",
      deleted: "Category deleted",
      hasFoods: "Cannot delete category with foods",
    },

    // ── Food ──────────────────────────────────────────────────
    food: {
      missingFields: "Missing required fields",
      invalidPrice: "Price must be a positive number",
      invalidTime: "Time must be a positive number",
      invalidCategory: "Invalid category",
      notFound: "Food not found",
      created: "Food created successfully",
      updated: "Food updated successfully",
      deleted: "Food deleted successfully",
      foodIdRequired: "foodId is required",
      // v4.2
      invalidMainCategory: "Invalid main category",
    },

    // ── Restaurant ────────────────────────────────────────────
    restaurant: {
      notFound: "Restaurant not found",
      updated: "Restaurant updated successfully",
      statusUpdated: "Restaurant status updated",
      blocked: "Restaurant is blocked and cannot change status",
      serverError: "Server error",
      invalidRestaurantId: "Invalid restaurantId",
      // v4.7
      invalidWorkingHours: "Please provide a valid opening and closing time",
      workingHoursUpdated: "Working hours updated",
    },

    // ── Orders ────────────────────────────────────────────────
    order: {
      notFound: "Order not found",
      allDeleted: "All orders deleted",
    },

    // ── Settlement ────────────────────────────────────────────
    settlement: {
      noAvailableBalance: "No available balance to withdraw",
      pendingExists: "You already have a pending withdrawal request",
      notFound: "Settlement not found",
      created: "Withdrawal request submitted",
      serverError: "Server error",
    },

    // ── Profile ───────────────────────────────────────────────
    profile: {
      notFound: "User not found",
      nothingToUpdate: "Nothing to update",
    },

    // ── General ───────────────────────────────────────────────
    general: {
      serverError: "Server error",
    },
  },

  ar: {
    auth: {
      userNotFound: "المستخدم غير موجود",
      phoneNotRegistered: "رقم الهاتف غير مسجل",
      emailNotRegistered: "البريد الإلكتروني غير مسجل",
      invalidPassword: "كلمة المرور غير صحيحة",
      invalidOtp: "رمز التحقق غير صحيح",
      otpExpired: "انتهت صلاحية رمز التحقق",
      phoneNotFound: "الهاتف غير موجود",
      emailNotFound: "البريد الإلكتروني غير موجود",
      invalidOrExpiredToken: "الرمز غير صالح أو منتهي الصلاحية",
      noRefreshToken: "لا يوجد رمز تحديث",
      invalidRefreshToken: "رمز التحديث غير صالح",
      refreshTokenExpired: "انتهت صلاحية رمز التحديث",
      missingFields: "حقول مطلوبة مفقودة",
      otpSentPhone: "تم إرسال رمز التحقق بنجاح",
      otpSentEmail: "تم إرسال رمز التحقق إلى بريدك الإلكتروني",
      passwordResetSuccess: "تم إعادة تعيين كلمة المرور بنجاح",
      resetLinkSent: "تم إرسال رابط إعادة التعيين إلى بريدك الإلكتروني",
      resetEmailFailed: "فشل في إرسال البريد الإلكتروني. يرجى المحاولة مجدداً.",
      loggedOut: "تم تسجيل الخروج",
      // v2.0 — إرسال SMS حقيقي عبر Aman Gate
      otpSendFailed: "فشل إرسال رمز التحقق. يرجى المحاولة مرة أخرى.",
      resendTooSoon: "الرجاء الانتظار قليلاً قبل طلب رمز جديد",
      resendSuccess: "تم إعادة إرسال رمز التحقق بنجاح",
      resendFailed: "فشل في إعادة إرسال رمز التحقق. حاول مرة أخرى.",
      invalidResendType: "نوع إعادة الإرسال غير صالح",
      // v4.2 — إعادة تعيين كلمة المرور بنظام الكود
      invalidResetType: "يجب أن يكون النوع 'phone' أو 'email'",
      passwordMismatch: "كلمتا المرور غير متطابقتين",
      passwordTooShort: "يجب أن تتكون كلمة المرور من 6 أحرف على الأقل",
      // v4.8 — حظر حساب المطعم من الأدمن
      accountBlocked: "تم حظر حسابك",
    },

    category: {
      nameRequired: "اسم التصنيف مطلوب",
      alreadyExists: "التصنيف موجود مسبقاً",
      notFound: "التصنيف غير موجود",
      created: "تم إنشاء التصنيف",
      updated: "تم تحديث التصنيف",
      deleted: "تم حذف التصنيف",
      hasFoods: "لا يمكن حذف تصنيف يحتوي على أطعمة",
    },

    food: {
      missingFields: "حقول مطلوبة مفقودة",
      invalidPrice: "يجب أن يكون السعر رقماً موجباً",
      invalidTime: "يجب أن يكون وقت التحضير رقماً موجباً",
      invalidCategory: "تصنيف غير صالح",
      notFound: "الطعام غير موجود",
      created: "تم إنشاء الطعام بنجاح",
      updated: "تم تحديث الطعام بنجاح",
      deleted: "تم حذف الطعام بنجاح",
      foodIdRequired: "معرّف الطعام مطلوب",
      // v4.2
      invalidMainCategory: "التصنيف الرئيسي غير صالح",
    },

    restaurant: {
      notFound: "المطعم غير موجود",
      updated: "تم تحديث المطعم بنجاح",
      statusUpdated: "تم تحديث حالة المطعم",
      blocked: "المطعم محظور ولا يمكن تغيير حالته",
      serverError: "خطأ في الخادم",
      invalidRestaurantId: "معرّف المطعم غير صالح",
      // v4.7
      invalidWorkingHours: "يرجى إدخال وقت فتح وإغلاق صحيحين",
      workingHoursUpdated: "تم تحديث ساعات العمل",
    },

    order: {
      notFound: "الطلب غير موجود",
      allDeleted: "تم حذف جميع الطلبات",
    },

    settlement: {
      noAvailableBalance: "لا يوجد رصيد متاح للسحب",
      pendingExists: "لديك طلب سحب قيد المراجعة بالفعل",
      notFound: "التسوية غير موجودة",
      created: "تم تقديم طلب السحب",
      serverError: "خطأ في الخادم",
    },

    profile: {
      notFound: "المستخدم غير موجود",
      nothingToUpdate: "لا يوجد شيء للتحديث",
    },

    general: {
      serverError: "خطأ في الخادم",
    },
  },

  de: {
    auth: {
      userNotFound: "Benutzer nicht gefunden",
      phoneNotRegistered: "Telefonnummer nicht registriert",
      emailNotRegistered: "E-Mail nicht registriert",
      invalidPassword: "Ungültiges Passwort",
      invalidOtp: "Ungültiger Verifizierungscode",
      otpExpired: "Verifizierungscode abgelaufen",
      phoneNotFound: "Telefonnummer nicht gefunden",
      emailNotFound: "E-Mail nicht gefunden",
      invalidOrExpiredToken: "Ungültiges oder abgelaufenes Token",
      noRefreshToken: "Kein Refresh-Token",
      invalidRefreshToken: "Ungültiges Refresh-Token",
      refreshTokenExpired: "Refresh-Token abgelaufen",
      missingFields: "Pflichtfelder fehlen",
      otpSentPhone: "Verifizierungscode erfolgreich gesendet",
      otpSentEmail: "Verifizierungscode an Ihre E-Mail gesendet",
      passwordResetSuccess: "Passwort erfolgreich zurückgesetzt",
      resetLinkSent: "Zurücksetzungslink an E-Mail gesendet",
      resetEmailFailed:
        "E-Mail konnte nicht gesendet werden. Bitte erneut versuchen.",
      loggedOut: "Abgemeldet",
      // v2.0 — echter SMS-Versand über Aman Gate
      otpSendFailed:
        "Verifizierungscode konnte nicht gesendet werden. Bitte versuche es erneut.",
      resendTooSoon: "Bitte warte kurz, bevor du einen neuen Code anforderst",
      resendSuccess: "Verifizierungscode erneut gesendet",
      resendFailed:
        "Erneutes Senden des Codes fehlgeschlagen. Bitte versuche es erneut.",
      invalidResendType: "Ungültiger Wiederholungstyp",
      // v4.2 — Passwort-Reset per Code
      invalidResetType: "Typ muss 'phone' oder 'email' sein",
      passwordMismatch: "Passwörter stimmen nicht überein",
      passwordTooShort: "Das Passwort muss mindestens 6 Zeichen lang sein",
      // v4.8 — Restaurantkonto vom Admin gesperrt
      accountBlocked: "Ihr Konto wurde gesperrt",
    },

    category: {
      nameRequired: "Kategoriename ist erforderlich",
      alreadyExists: "Kategorie existiert bereits",
      notFound: "Kategorie nicht gefunden",
      created: "Kategorie erstellt",
      updated: "Kategorie aktualisiert",
      deleted: "Kategorie gelöscht",
      hasFoods: "Kategorie mit Speisen kann nicht gelöscht werden",
    },

    food: {
      missingFields: "Pflichtfelder fehlen",
      invalidPrice: "Preis muss eine positive Zahl sein",
      invalidTime: "Zubereitungszeit muss eine positive Zahl sein",
      invalidCategory: "Ungültige Kategorie",
      notFound: "Speise nicht gefunden",
      created: "Speise erfolgreich erstellt",
      updated: "Speise erfolgreich aktualisiert",
      deleted: "Speise erfolgreich gelöscht",
      foodIdRequired: "Speisen-ID ist erforderlich",
      // v4.2
      invalidMainCategory: "Ungültige Hauptkategorie",
    },

    restaurant: {
      notFound: "Restaurant nicht gefunden",
      updated: "Restaurant erfolgreich aktualisiert",
      statusUpdated: "Restaurantstatus aktualisiert",
      blocked: "Restaurant ist gesperrt und kann den Status nicht ändern",
      serverError: "Serverfehler",
      invalidRestaurantId: "Ungültige Restaurant-ID",
      // v4.7
      invalidWorkingHours:
        "Bitte gib eine gültige Öffnungs- und Schließzeit an",
      workingHoursUpdated: "Öffnungszeiten aktualisiert",
    },

    order: {
      notFound: "Bestellung nicht gefunden",
      allDeleted: "Alle Bestellungen gelöscht",
    },

    settlement: {
      noAvailableBalance: "Kein verfügbares Guthaben zur Auszahlung",
      pendingExists: "Sie haben bereits eine ausstehende Auszahlungsanfrage",
      notFound: "Abrechnung nicht gefunden",
      created: "Auszahlungsanfrage eingereicht",
      serverError: "Serverfehler",
    },

    profile: {
      notFound: "Benutzer nicht gefunden",
      nothingToUpdate: "Nichts zu aktualisieren",
    },

    general: {
      serverError: "Serverfehler",
    },
  },
};

// ─────────────────────────────────────────────────────────────
// USER & DRIVER TRANSLATIONS
// مُضاف منفصلاً حتى لو تشابهت النصوص مع المطعم
// ─────────────────────────────────────────────────────────────

// v4.2 — الكتلة القديمة translations.en/ar/de.user التي كانت هنا حُذفت.
// كانت مُعادة الإسناد لاحقاً بنفس الملف (translations.<lang>.user = {...}
// تظهر مرتين) فتُلغى صامتاً بلا أي خطأ من Node — الإسناد الثاني هو الفعلي
// الوحيد المُصدَّر. عند حذف هذي الكتلة الميتة، انتقلت أي مفاتيح كانت موجودة
// فيها فقط (rating.invalidOrderId/orderNotFound/noDriver/driverNotFound)
// إلى الكتلة الحية أدناه، بدل ما تضيع. لا تُعاد كتابة "translations.<lang>.user ="
// أكثر من مرة بهذا الملف — هذا بالضبط سبب المشكلة الأصلية.

// ─── DRIVER ───────────────────────────────────────────────────

translations.en.driver = {
  auth: {
    missingFields: "Missing required fields",
    invalidDePhone: "Invalid German phone number. Must start with +49",
    invalidSyPhone: "Invalid Syrian phone number. Must start with +963",
    invalidPhone: "Phone must start with +49 (Germany) or +963 (Syria)",
    invalidVehicleType: "Invalid vehicle type",
    emailOrPhoneExists: "Email or phone already exists",
    documentsRequired:
      "ID, driving license, and vehicle registration images are required",
    driverCreated: "Driver created successfully",
    invalidPhoneOrPass: "Invalid phone or password",
    accountBlocked: "Your account has been blocked",
    accountRejected: "Your account has been rejected",
    // v4.2
    accountPending: "Your account is still under review",
    otpSent: "OTP sent",
    phoneNotFound: "Phone not found",
    noOtpRequested: "No OTP was requested",
    otpExpired: "OTP expired",
    invalidOtp: "Invalid OTP",
    emailNotFound: "Email not found",
    resetLinkSent: "A password reset link has been sent to your email",
    resetEmailFailed: "Failed to send reset email. Please try again.",
    invalidOrExpiredToken: "Invalid or expired token",
    passwordResetSuccess: "Password reset successful",
    // v2.0 — SMS OTP (Aman Gate) real sending
    otpSendFailed: "Failed to send verification code. Please try again.",
    resendTooSoon: "Please wait a bit before requesting another code",
    // v4.3 — إعادة تعيين كلمة المرور بنظام الكود
    passwordMismatch: "Passwords do not match",
    passwordTooShort: "Password must be at least 6 characters",
  },
  info: {
    notFound: "Driver not found",
    nameEmpty: "Name cannot be empty",
    invalidVehicleType: "Invalid vehicle type",
    plateEmpty: "Vehicle plate cannot be empty",
    updated: "Updated successfully",
    passwordRequired: "currentPassword and newPassword are required",
    passwordTooShort: "New password must be at least 6 characters",
    passwordSame: "New password must be different from current password",
    incorrectPassword: "Current password is incorrect",
    passwordUpdated: "Password updated successfully",
  },
  order: {
    notFound: "Order not found",
    fcmRequired: "fcmToken is required",
    fcmUpdated: "FCM token updated",
    loggedOut: "Logged out successfully",
  },
  general: {
    serverError: "Server error",
  },
};

translations.ar.driver = {
  auth: {
    missingFields: "حقول مطلوبة مفقودة",
    invalidDePhone: "رقم هاتف ألماني غير صالح. يجب أن يبدأ بـ +49",
    invalidSyPhone: "رقم هاتف سوري غير صالح. يجب أن يبدأ بـ +963",
    invalidPhone: "يجب أن يبدأ الهاتف بـ +49 (ألمانيا) أو +963 (سوريا)",
    invalidVehicleType: "نوع المركبة غير صالح",
    emailOrPhoneExists: "البريد الإلكتروني أو الهاتف مستخدم بالفعل",
    documentsRequired: "صورة الهوية ورخصة القيادة وأوراق المركبة كلها مطلوبة",
    driverCreated: "تم تسجيل السائق بنجاح",
    invalidPhoneOrPass: "رقم الهاتف أو كلمة المرور غير صحيحة",
    accountBlocked: "تم حظر حسابك",
    accountRejected: "تم رفض حسابك",
    // v4.2
    accountPending: "حسابك لا يزال قيد المراجعة",
    otpSent: "تم إرسال رمز التحقق",
    phoneNotFound: "رقم الهاتف غير موجود",
    noOtpRequested: "لم يتم طلب رمز تحقق",
    otpExpired: "انتهت صلاحية رمز التحقق",
    invalidOtp: "رمز التحقق غير صحيح",
    emailNotFound: "البريد الإلكتروني غير موجود",
    resetLinkSent: "تم إرسال رابط إعادة تعيين كلمة المرور إلى بريدك الإلكتروني",
    resetEmailFailed: "فشل إرسال البريد الإلكتروني. يرجى المحاولة مجدداً.",
    invalidOrExpiredToken: "الرابط غير صالح أو منتهي الصلاحية",
    passwordResetSuccess: "تم إعادة تعيين كلمة المرور بنجاح",
    // v2.0 — إرسال SMS حقيقي عبر Aman Gate
    otpSendFailed: "فشل إرسال رمز التحقق. يرجى المحاولة مرة أخرى.",
    resendTooSoon: "الرجاء الانتظار قليلاً قبل طلب رمز جديد",
    // v4.3 — إعادة تعيين كلمة المرور بنظام الكود
    passwordMismatch: "كلمتا المرور غير متطابقتين",
    passwordTooShort: "يجب أن تتكون كلمة المرور من 6 أحرف على الأقل",
  },
  info: {
    notFound: "السائق غير موجود",
    nameEmpty: "الاسم لا يمكن أن يكون فارغاً",
    invalidVehicleType: "نوع المركبة غير صالح",
    plateEmpty: "لوحة المركبة لا يمكن أن تكون فارغة",
    updated: "تم التحديث بنجاح",
    passwordRequired: "كلمة المرور الحالية والجديدة مطلوبتان",
    passwordTooShort: "يجب أن تكون كلمة المرور الجديدة 6 أحرف على الأقل",
    passwordSame: "يجب أن تختلف كلمة المرور الجديدة عن الحالية",
    incorrectPassword: "كلمة المرور الحالية غير صحيحة",
    passwordUpdated: "تم تحديث كلمة المرور بنجاح",
  },
  order: {
    notFound: "الطلب غير موجود",
    fcmRequired: "رمز FCM مطلوب",
    fcmUpdated: "تم تحديث رمز FCM بنجاح",
    loggedOut: "تم تسجيل الخروج بنجاح",
  },
  general: {
    serverError: "خطأ في الخادم",
  },
};

translations.de.driver = {
  auth: {
    missingFields: "Pflichtfelder fehlen",
    invalidDePhone: "Ungültige deutsche Telefonnummer. Muss mit +49 beginnen",
    invalidSyPhone: "Ungültige syrische Telefonnummer. Muss mit +963 beginnen",
    invalidPhone:
      "Telefon muss mit +49 (Deutschland) oder +963 (Syrien) beginnen",
    invalidVehicleType: "Ungültiger Fahrzeugtyp",
    emailOrPhoneExists: "E-Mail oder Telefonnummer bereits vergeben",
    documentsRequired:
      "Ausweisbild, Führerschein und Fahrzeugschein sind erforderlich",
    driverCreated: "Fahrer erfolgreich registriert",
    invalidPhoneOrPass: "Ungültige Telefonnummer oder Passwort",
    accountBlocked: "Dein Konto wurde gesperrt",
    accountRejected: "Dein Konto wurde abgelehnt",
    // v4.2
    accountPending: "Dein Konto wird noch überprüft",
    otpSent: "Bestätigungscode gesendet",
    phoneNotFound: "Telefonnummer nicht gefunden",
    noOtpRequested: "Kein Bestätigungscode angefordert",
    otpExpired: "Bestätigungscode abgelaufen",
    invalidOtp: "Ungültiger Bestätigungscode",
    emailNotFound: "E-Mail nicht gefunden",
    resetLinkSent:
      "Ein Passwort-Zurücksetzungslink wurde an deine E-Mail gesendet",
    resetEmailFailed:
      "E-Mail konnte nicht gesendet werden. Bitte versuche es erneut.",
    invalidOrExpiredToken: "Ungültiger oder abgelaufener Link",
    passwordResetSuccess: "Passwort erfolgreich zurückgesetzt",
    // v2.0 — echter SMS-Versand über Aman Gate
    otpSendFailed:
      "Verifizierungscode konnte nicht gesendet werden. Bitte versuche es erneut.",
    resendTooSoon: "Bitte warte kurz, bevor du einen neuen Code anforderst",
    // v4.3 — Passwort-Reset mit Code-System
    passwordMismatch: "Passwörter stimmen nicht überein",
    passwordTooShort: "Das Passwort muss mindestens 6 Zeichen lang sein",
  },
  info: {
    notFound: "Fahrer nicht gefunden",
    nameEmpty: "Name darf nicht leer sein",
    invalidVehicleType: "Ungültiger Fahrzeugtyp",
    plateEmpty: "Kennzeichen darf nicht leer sein",
    updated: "Erfolgreich aktualisiert",
    passwordRequired: "Aktuelles und neues Passwort sind erforderlich",
    passwordTooShort: "Neues Passwort muss mindestens 6 Zeichen haben",
    passwordSame: "Neues Passwort muss sich vom aktuellen unterscheiden",
    incorrectPassword: "Aktuelles Passwort ist falsch",
    passwordUpdated: "Passwort erfolgreich aktualisiert",
  },
  order: {
    notFound: "Bestellung nicht gefunden",
    fcmRequired: "FCM-Token ist erforderlich",
    fcmUpdated: "FCM-Token erfolgreich aktualisiert",
    loggedOut: "Erfolgreich abgemeldet",
  },
  general: {
    serverError: "Serverfehler",
  },
};

// ─────────────────────────────────────────────────────────────
// UPLOAD TRANSLATIONS — v2.0 (B3): مستخدمة عبر upload.safe
// ─────────────────────────────────────────────────────────────

translations.en.upload = {
  fileTooLarge: "Image is too large. Maximum size is {{max}} MB",
  invalidFileType: "Only image files are allowed",
  invalidUpload: "Invalid file upload",
};
translations.ar.upload = {
  fileTooLarge: "حجم الصورة كبير. الحد الأقصى {{max}} ميجابايت",
  invalidFileType: "يُسمح بملفات الصور فقط",
  invalidUpload: "ملف مرفوع غير صالح",
};
translations.de.upload = {
  fileTooLarge: "Bild ist zu groß. Maximal {{max}} MB",
  invalidFileType: "Nur Bilddateien sind erlaubt",
  invalidUpload: "Ungültiger Datei-Upload",
};

// ─────────────────────────────────────────────────────────────
// SOCKET TRANSLATIONS
// ─────────────────────────────────────────────────────────────

translations.en.socket = {
  driver: {
    hasActiveOrder: "You have an active order — complete it first",
    orderIdAndResponse: "orderId and response are required",
    rejected: "You rejected the order",
    orderNotFound: "Order not found",
    notOnline: "You cannot accept orders while busy or offline",
    alreadyHasOrder: "You already have an active order",
    orderExpired: "This order is no longer available",
    orderTaken: "Order already taken by another driver",
    differentCountry: "You cannot accept orders from a different country",
    cashLimitReached:
      "Cannot accept order — cash limit reached. Please settle your collected cash first.",
    orderIdRequired: "orderId is required",
    mustBePickedUp: "Order must be picked_up first",
    mustBeOnTheWay: "Order must be on_the_way first",
  },
  user: {
    orderIdRequired: "orderId is required",
    orderNotFound: "Order not found",
    alreadySentOrCancelled: "Order already sent or cancelled",
    restaurantClosed: "Restaurant is currently closed",
    paymentRequired: "Payment is required before sending the order",
    paymentNotCompleted:
      "Payment not completed. Please complete payment first.",
    orderSent: "Order sent to restaurant",
  },
  restaurant: {
    orderIdRequired: "orderId is required",
    invalidStatus: "Invalid status",
    orderNotFound: "Order not found",
    cannotChangeStatus: "Cannot change status: order is already {{status}}",
    searchingDriver: "Searching for a driver",
    notSearchable: "Order is not in a searchable state",
    // v4.4
    cancellationReasonRequired: "A cancellation reason is required",
    invalidCancellationReason: "Invalid cancellation reason",
    cancellationNoteRequired: "Please describe the reason",
  },
};

translations.ar.socket = {
  driver: {
    hasActiveOrder: "لديك طلب نشط — أكمله أولاً",
    orderIdAndResponse: "معرّف الطلب والرد مطلوبان",
    rejected: "لقد رفضت الطلب",
    orderNotFound: "الطلب غير موجود",
    notOnline: "لا يمكنك قبول الطلبات وأنت مشغول أو غير متصل",
    alreadyHasOrder: "لديك طلب نشط بالفعل",
    orderExpired: "هذا الطلب لم يعد متاحاً",
    orderTaken: "تم أخذ الطلب من قبل سائق آخر",
    differentCountry: "لا يمكنك قبول طلبات من بلد مختلف",
    cashLimitReached:
      "لا يمكن قبول الطلب — تم بلوغ حد الكاش. يرجى تسوية المبالغ المحصّلة أولاً.",
    orderIdRequired: "معرّف الطلب مطلوب",
    mustBePickedUp: "يجب أن يكون الطلب في حالة استلام أولاً",
    mustBeOnTheWay: "يجب أن يكون الطلب في الطريق أولاً",
  },
  user: {
    orderIdRequired: "معرّف الطلب مطلوب",
    orderNotFound: "الطلب غير موجود",
    alreadySentOrCancelled: "تم إرسال الطلب أو إلغاؤه بالفعل",
    restaurantClosed: "المطعم مغلق حالياً",
    paymentRequired: "يجب إتمام الدفع قبل إرسال الطلب",
    paymentNotCompleted: "لم يكتمل الدفع. يرجى إتمام الدفع أولاً.",
    orderSent: "تم إرسال الطلب إلى المطعم",
  },
  restaurant: {
    orderIdRequired: "معرّف الطلب مطلوب",
    invalidStatus: "حالة غير صالحة",
    orderNotFound: "الطلب غير موجود",
    cannotChangeStatus: "لا يمكن تغيير الحالة: الطلب بالفعل في حالة {{status}}",
    searchingDriver: "جارٍ البحث عن سائق",
    notSearchable: "الطلب ليس في حالة قابلة للبحث",
    // v4.4
    cancellationReasonRequired: "سبب الإلغاء مطلوب",
    invalidCancellationReason: "سبب إلغاء غير صالح",
    cancellationNoteRequired: "يرجى كتابة السبب",
  },
};

translations.de.socket = {
  driver: {
    hasActiveOrder: "Du hast eine aktive Bestellung — schließe sie zuerst ab",
    orderIdAndResponse: "Bestell-ID und Antwort sind erforderlich",
    rejected: "Du hast die Bestellung abgelehnt",
    orderNotFound: "Bestellung nicht gefunden",
    notOnline:
      "Du kannst keine Bestellungen annehmen, während du beschäftigt oder offline bist",
    alreadyHasOrder: "Du hast bereits eine aktive Bestellung",
    orderExpired: "Diese Bestellung ist nicht mehr verfügbar",
    orderTaken: "Bestellung wurde bereits von einem anderen Fahrer angenommen",
    differentCountry:
      "Du kannst keine Bestellungen aus einem anderen Land annehmen",
    cashLimitReached:
      "Bestellung kann nicht angenommen werden — Barlimit erreicht. Bitte verrechne zuerst dein gesammeltes Bargeld.",
    orderIdRequired: "Bestell-ID ist erforderlich",
    mustBePickedUp: "Bestellung muss zuerst abgeholt werden",
    mustBeOnTheWay: "Bestellung muss sich zuerst auf dem Weg befinden",
  },
  user: {
    orderIdRequired: "Bestell-ID ist erforderlich",
    orderNotFound: "Bestellung nicht gefunden",
    alreadySentOrCancelled: "Bestellung bereits gesendet oder storniert",
    restaurantClosed: "Das Restaurant ist derzeit geschlossen",
    paymentRequired: "Zahlung ist vor dem Senden der Bestellung erforderlich",
    paymentNotCompleted:
      "Zahlung nicht abgeschlossen. Bitte schließe die Zahlung zuerst ab.",
    orderSent: "Bestellung wurde an das Restaurant gesendet",
  },
  restaurant: {
    orderIdRequired: "Bestell-ID ist erforderlich",
    invalidStatus: "Ungültiger Status",
    orderNotFound: "Bestellung nicht gefunden",
    cannotChangeStatus:
      "Status kann nicht geändert werden: Bestellung ist bereits {{status}}",
    searchingDriver: "Suche nach einem Fahrer",
    notSearchable: "Bestellung befindet sich nicht in einem suchbaren Zustand",
    // v4.4
    cancellationReasonRequired: "Ein Stornierungsgrund ist erforderlich",
    invalidCancellationReason: "Ungültiger Stornierungsgrund",
    cancellationNoteRequired: "Bitte beschreibe den Grund",
  },
};

translations.en.user = {
  auth: {
    userNotFound: "User not found",
    phoneNotFound: "Phone not found",
    emailNotFound: "Email not found",
    missingFields: "Missing required fields",
    invalidPhone: "Phone must start with +49 (Germany) or +963 (Syria)",
    invalidDePhone: "Invalid German phone number. Must start with +49",
    invalidSyPhone: "Invalid Syrian phone number. Must start with +963",
    invalidGender: "Invalid gender",
    emailUsed: "Email already in use",
    phoneUsed: "Phone already in use",
    userCreated: "Account created successfully",
    invalidPhoneOrPass: "Invalid phone number or password",
    invalidEmailOrPass: "Invalid email or password",
    accountBlocked: "Your account has been blocked",
    otpSent: "OTP sent successfully",
    otpSentEmail: "OTP sent to your email",
    invalidOtp: "Invalid OTP",
    otpExpired: "OTP expired",
    invalidOrExpiredToken: "Invalid or expired token",
    passwordResetSuccess: "Password reset successful",
    resetLinkSent: "Reset link sent to email",
    resetEmailFailed: "Failed to send reset email. Please try again.",
    profileUpdated: "Profile updated successfully",
    // v2.0 — SMS OTP (Aman Gate) real sending
    otpSendFailed: "Failed to send verification code. Please try again.",
    resendTooSoon: "Please wait a bit before requesting another code",
    // v4.3 — إعادة تعيين كلمة المرور بنظام الكود
    passwordMismatch: "Passwords do not match",
    passwordTooShort: "Password must be at least 6 characters",
  },
  address: {
    missingFields: "Missing required fields",
    maxAddresses: "Maximum number of addresses reached",
    added: "Address added successfully",
    notFound: "Address not found",
    idRequired: "Address ID is required",
    updated: "Address updated successfully",
    defaultUpdated: "Default address updated",
    deleted: "Address deleted successfully",
    cannotDeleteOnly: "Cannot delete the only address",
    userNotFound: "User not found",
  },
  cart: {
    cartNotFound: "Cart not found",
    foodNotAvailable: "This item is not available",
    differentRestaurant: "Cannot add items from different restaurants",
    noSizes: "This item has no sizes",
    sizeRequired: "Please select a size",
    invalidSize: "Invalid size selected",
    invalidQuantity: "Invalid quantity",
    added: "Item added to cart",
    itemNotFound: "Item not found in cart",
    itemRemoved: "Item removed from cart",
    cleared: "Cart cleared",
    fetchFailed: "Failed to fetch cart",
    removeFailed: "Failed to remove item",
    // v3.9 — coupons
    couponRequired: "Coupon code is required",
    couponInvalid: "Invalid or inactive coupon code",
    couponExpired: "This coupon has expired",
    couponCountryNotSupported: "Coupons are not available for your country yet",
    couponNotEligible: "This coupon is not available for your account",
    couponLimitReached: "This coupon has reached its usage limit",
    couponUserLimitReached:
      "You've already used this coupon the maximum number of times",
    couponMinOrderNotMet: "This coupon requires a minimum order of {{amount}}",
    couponApplied: "Coupon applied successfully",
    couponRemoved: "Coupon removed",
    // v4.2
    invalidExtra: "One or more selected extras are not available for this item",
    quantityUpdated: "Quantity updated successfully",
    cartEmpty: "Your cart is empty",
  },
  order: {
    cartEmpty: "Your cart is empty",
    restaurantNotFound: "Restaurant not found",
    restaurantClosed: "Restaurant is currently closed",
    differentCountry: "Cannot order from a different country",
    paymentNotAccepted: "Payment method not accepted",
    accountBlocked: "Your account has been blocked",
    // v4.2
    invalidDeliveryAddress:
      "Please provide a valid delivery address and location",
    created: "Order created successfully",
    notFound: "Order not found",
    orderIdRequired: "Order ID is required",
    cannotCancel: "Order cannot be cancelled at this stage",
    cancelled: "Order cancelled successfully",
    deleted: "Order deleted successfully",
    fcmRequired: "fcmToken is required",
    fcmUpdated: "FCM token updated",
    loggedOut: "Logged out successfully",
  },
  payment: {
    cartEmpty: "Your cart is empty",
    restaurantNotFound: "Restaurant not found",
    germanOnly: "Online payment is only available for German orders",
  },
  ad: {
    adIdRequired: "Ad ID is required",
    notFound: "Ad not found",
  },
  favorite: {
    userNotFound: "User not found",
    foodIdRequired: "Food ID is required",
    foodAdded: "Food added to favorites",
    foodRemoved: "Food removed from favorites",
    restaurantIdRequired: "Restaurant ID is required",
    restaurantAdded: "Restaurant added to favorites",
    restaurantRemoved: "Restaurant removed from favorites",
  },
  rating: {
    invalidFoodId: "Invalid food ID",
    foodNotFound: "Food not found",
    invalidRating: "Rating must be between 1 and 5",
    rated: "Rated successfully",
    // v4.2 — استُعيدت من الكتلة الميتة القديمة
    invalidOrderId: "Invalid orderId",
    orderNotFound: "Order not found or not delivered yet",
    noDriver: "No driver assigned to this order",
    driverNotFound: "Driver not found",
  },
  general: {
    serverError: "Server error",
  },
};

translations.ar.user = {
  auth: {
    userNotFound: "المستخدم غير موجود",
    phoneNotFound: "الهاتف غير موجود",
    emailNotFound: "البريد الإلكتروني غير موجود",
    missingFields: "حقول مطلوبة مفقودة",
    invalidPhone: "يجب أن يبدأ الهاتف بـ +49 (ألمانيا) أو +963 (سوريا)",
    invalidDePhone: "رقم هاتف ألماني غير صالح. يجب أن يبدأ بـ +49",
    invalidSyPhone: "رقم هاتف سوري غير صالح. يجب أن يبدأ بـ +963",
    invalidGender: "جنس غير صالح",
    emailUsed: "البريد الإلكتروني مستخدم بالفعل",
    phoneUsed: "رقم الهاتف مستخدم بالفعل",
    userCreated: "تم إنشاء الحساب بنجاح",
    invalidPhoneOrPass: "رقم الهاتف أو كلمة المرور غير صحيحة",
    invalidEmailOrPass: "البريد الإلكتروني أو كلمة المرور غير صحيحة",
    accountBlocked: "تم حظر حسابك",
    otpSent: "تم إرسال رمز التحقق بنجاح",
    otpSentEmail: "تم إرسال رمز التحقق إلى بريدك الإلكتروني",
    invalidOtp: "رمز التحقق غير صحيح",
    otpExpired: "انتهت صلاحية رمز التحقق",
    invalidOrExpiredToken: "الرمز غير صالح أو منتهي الصلاحية",
    passwordResetSuccess: "تم إعادة تعيين كلمة المرور بنجاح",
    resetLinkSent: "تم إرسال رابط إعادة التعيين إلى بريدك الإلكتروني",
    resetEmailFailed: "فشل في إرسال البريد الإلكتروني. يرجى المحاولة مجدداً.",
    profileUpdated: "تم تحديث الملف الشخصي بنجاح",
    // v2.0 — إرسال SMS حقيقي عبر Aman Gate
    otpSendFailed: "فشل إرسال رمز التحقق. يرجى المحاولة مرة أخرى.",
    resendTooSoon: "الرجاء الانتظار قليلاً قبل طلب رمز جديد",
    // v4.3 — إعادة تعيين كلمة المرور بنظام الكود
    passwordMismatch: "كلمتا المرور غير متطابقتين",
    passwordTooShort: "يجب أن تتكون كلمة المرور من 6 أحرف على الأقل",
  },
  address: {
    missingFields: "حقول مطلوبة مفقودة",
    maxAddresses: "تم الوصول إلى الحد الأقصى للعناوين",
    added: "تم إضافة العنوان بنجاح",
    notFound: "العنوان غير موجود",
    idRequired: "معرّف العنوان مطلوب",
    updated: "تم تحديث العنوان بنجاح",
    defaultUpdated: "تم تحديث العنوان الافتراضي",
    deleted: "تم حذف العنوان بنجاح",
    cannotDeleteOnly: "لا يمكن حذف العنوان الوحيد",
    userNotFound: "المستخدم غير موجود",
  },
  cart: {
    cartNotFound: "السلة غير موجودة",
    foodNotAvailable: "هذا العنصر غير متاح حالياً",
    differentRestaurant: "لا يمكن إضافة عناصر من مطاعم مختلفة",
    noSizes: "هذا العنصر لا يحتوي على أحجام",
    sizeRequired: "يرجى اختيار حجم",
    invalidSize: "الحجم المختار غير صالح",
    invalidQuantity: "الكمية غير صالحة",
    added: "تم إضافة العنصر إلى السلة",
    itemNotFound: "العنصر غير موجود في السلة",
    itemRemoved: "تم إزالة العنصر من السلة",
    cleared: "تم تفريغ السلة",
    fetchFailed: "فشل في جلب السلة",
    removeFailed: "فشل في إزالة العنصر",
    // v3.9 — coupons
    couponRequired: "كود الكوبون مطلوب",
    couponInvalid: "كود الكوبون غير صالح أو غير مفعّل",
    couponExpired: "انتهت صلاحية هذا الكوبون",
    couponCountryNotSupported: "الكوبونات غير متاحة لبلدك حالياً",
    couponNotEligible: "هذا الكوبون غير متاح لحسابك",
    couponLimitReached: "وصل هذا الكوبون للحد الأقصى من الاستخدام",
    couponUserLimitReached:
      "لقد استخدمت هذا الكوبون الحد الأقصى المسموح به من المرات",
    couponMinOrderNotMet: "هذا الكوبون يتطلب حد أدنى للطلب قيمته {{amount}}",
    couponApplied: "تم تطبيق الكوبون بنجاح",
    couponRemoved: "تم إزالة الكوبون",
    // v4.2
    invalidExtra: "إحدى الإضافات المختارة غير متاحة لهذه الوجبة",
    quantityUpdated: "تم تحديث الكمية بنجاح",
    cartEmpty: "سلتك فارغة",
  },
  order: {
    cartEmpty: "سلتك فارغة",
    restaurantNotFound: "المطعم غير موجود",
    restaurantClosed: "المطعم مغلق حالياً",
    differentCountry: "لا يمكن الطلب من بلد مختلف",
    paymentNotAccepted: "طريقة الدفع غير مقبولة",
    accountBlocked: "تم حظر حسابك",
    // v4.2
    invalidDeliveryAddress: "الرجاء إدخال عنوان توصيل وموقع صالحين",
    created: "تم إنشاء الطلب بنجاح",
    notFound: "الطلب غير موجود",
    orderIdRequired: "معرّف الطلب مطلوب",
    cannotCancel: "لا يمكن إلغاء الطلب في هذه المرحلة",
    cancelled: "تم إلغاء الطلب بنجاح",
    deleted: "تم حذف الطلب بنجاح",
    fcmRequired: "رمز FCM مطلوب",
    fcmUpdated: "تم تحديث رمز FCM بنجاح",
    loggedOut: "تم تسجيل الخروج بنجاح",
  },
  payment: {
    cartEmpty: "سلتك فارغة",
    restaurantNotFound: "المطعم غير موجود",
    germanOnly: "الدفع الإلكتروني متاح للطلبات الألمانية فقط",
  },
  ad: {
    adIdRequired: "معرّف الإعلان مطلوب",
    notFound: "الإعلان غير موجود",
  },
  favorite: {
    userNotFound: "المستخدم غير موجود",
    foodIdRequired: "معرّف الطعام مطلوب",
    foodAdded: "تم إضافة الطعام إلى المفضلة",
    foodRemoved: "تم إزالة الطعام من المفضلة",
    restaurantIdRequired: "معرّف المطعم مطلوب",
    restaurantAdded: "تم إضافة المطعم إلى المفضلة",
    restaurantRemoved: "تم إزالة المطعم من المفضلة",
  },
  rating: {
    invalidFoodId: "معرّف الطعام غير صالح",
    foodNotFound: "الطعام غير موجود",
    invalidRating: "يجب أن يكون التقييم بين 1 و 5",
    rated: "تم التقييم بنجاح",
    // v4.2 — استُعيدت من الكتلة الميتة القديمة
    invalidOrderId: "معرّف الطلب غير صالح",
    orderNotFound: "الطلب غير موجود أو لم يتم توصيله بعد",
    noDriver: "لا يوجد سائق مرتبط بهذا الطلب",
    driverNotFound: "السائق غير موجود",
  },
  general: {
    serverError: "خطأ في الخادم",
  },
};

translations.de.user = {
  auth: {
    userNotFound: "Benutzer nicht gefunden",
    phoneNotFound: "Telefonnummer nicht gefunden",
    emailNotFound: "E-Mail nicht gefunden",
    missingFields: "Pflichtfelder fehlen",
    invalidPhone:
      "Telefon muss mit +49 (Deutschland) oder +963 (Syrien) beginnen",
    invalidDePhone: "Ungültige deutsche Telefonnummer. Muss mit +49 beginnen",
    invalidSyPhone: "Ungültige syrische Telefonnummer. Muss mit +963 beginnen",
    invalidGender: "Ungültiges Geschlecht",
    emailUsed: "E-Mail bereits vergeben",
    phoneUsed: "Telefonnummer bereits vergeben",
    userCreated: "Konto erfolgreich erstellt",
    invalidPhoneOrPass: "Ungültige Telefonnummer oder Passwort",
    invalidEmailOrPass: "Ungültige E-Mail oder Passwort",
    accountBlocked: "Ihr Konto wurde gesperrt",
    otpSent: "Bestätigungscode gesendet",
    otpSentEmail: "Bestätigungscode an Ihre E-Mail gesendet",
    invalidOtp: "Ungültiger Bestätigungscode",
    otpExpired: "Bestätigungscode abgelaufen",
    invalidOrExpiredToken: "Ungültiges oder abgelaufenes Token",
    passwordResetSuccess: "Passwort erfolgreich zurückgesetzt",
    resetLinkSent: "Zurücksetzungslink an E-Mail gesendet",
    resetEmailFailed:
      "E-Mail konnte nicht gesendet werden. Bitte erneut versuchen.",
    profileUpdated: "Profil erfolgreich aktualisiert",
    // v2.0 — SMS OTP (Aman Gate) real sending
    resendTooSoon: "Bitte warte kurz, bevor du einen neuen Code anforderst",
    // v4.3 — Passwort-Reset mit Code-System
    passwordMismatch: "Passwörter stimmen nicht überein",
    passwordTooShort: "Das Passwort muss mindestens 6 Zeichen lang sein",
  },
  address: {
    missingFields: "Pflichtfelder fehlen",
    maxAddresses: "Maximale Anzahl an Adressen erreicht",
    added: "Adresse erfolgreich hinzugefügt",
    notFound: "Adresse nicht gefunden",
    idRequired: "Adress-ID ist erforderlich",
    updated: "Adresse erfolgreich aktualisiert",
    defaultUpdated: "Standardadresse aktualisiert",
    deleted: "Adresse erfolgreich gelöscht",
    cannotDeleteOnly: "Die einzige Adresse kann nicht gelöscht werden",
    userNotFound: "Benutzer nicht gefunden",
  },
  cart: {
    cartNotFound: "Warenkorb nicht gefunden",
    foodNotAvailable: "Dieser Artikel ist nicht verfügbar",
    differentRestaurant:
      "Artikel aus verschiedenen Restaurants können nicht hinzugefügt werden",
    noSizes: "Dieser Artikel hat keine Größen",
    sizeRequired: "Bitte wählen Sie eine Größe",
    invalidSize: "Ungültige Größe ausgewählt",
    invalidQuantity: "Ungültige Menge",
    added: "Artikel zum Warenkorb hinzugefügt",
    itemNotFound: "Artikel nicht im Warenkorb gefunden",
    itemRemoved: "Artikel aus dem Warenkorb entfernt",
    cleared: "Warenkorb geleert",
    fetchFailed: "Warenkorb konnte nicht geladen werden",
    removeFailed: "Artikel konnte nicht entfernt werden",
    // v3.9 — coupons
    couponRequired: "Gutscheincode ist erforderlich",
    couponInvalid: "Ungültiger oder inaktiver Gutscheincode",
    couponExpired: "Dieser Gutschein ist abgelaufen",
    couponCountryNotSupported:
      "Gutscheine sind für Ihr Land noch nicht verfügbar",
    couponNotEligible: "Dieser Gutschein ist für Ihr Konto nicht verfügbar",
    couponLimitReached: "Dieser Gutschein hat sein Nutzungslimit erreicht",
    couponUserLimitReached:
      "Sie haben diesen Gutschein bereits maximal oft verwendet",
    couponMinOrderNotMet:
      "Dieser Gutschein erfordert einen Mindestbestellwert von {{amount}}",
    couponApplied: "Gutschein erfolgreich angewendet",
    couponRemoved: "Gutschein entfernt",
    // v4.2
    invalidExtra:
      "Ein oder mehrere ausgewählte Extras sind für diesen Artikel nicht verfügbar",
    quantityUpdated: "Menge erfolgreich aktualisiert",
    cartEmpty: "Ihr Warenkorb ist leer",
  },
  order: {
    cartEmpty: "Ihr Warenkorb ist leer",
    restaurantNotFound: "Restaurant nicht gefunden",
    restaurantClosed: "Das Restaurant ist derzeit geschlossen",
    differentCountry: "Bestellung aus einem anderen Land nicht möglich",
    paymentNotAccepted: "Zahlungsmethode nicht akzeptiert",
    accountBlocked: "Ihr Konto wurde gesperrt",
    // v4.2
    invalidDeliveryAddress:
      "Bitte geben Sie eine gültige Lieferadresse und einen gültigen Standort an",
    created: "Bestellung erfolgreich erstellt",
    notFound: "Bestellung nicht gefunden",
    orderIdRequired: "Bestell-ID ist erforderlich",
    cannotCancel: "Bestellung kann in diesem Stadium nicht storniert werden",
    cancelled: "Bestellung erfolgreich storniert",
    deleted: "Bestellung erfolgreich gelöscht",
    fcmRequired: "FCM-Token ist erforderlich",
    fcmUpdated: "FCM-Token erfolgreich aktualisiert",
    loggedOut: "Erfolgreich abgemeldet",
  },
  payment: {
    cartEmpty: "Ihr Warenkorb ist leer",
    restaurantNotFound: "Restaurant nicht gefunden",
    germanOnly: "Online-Zahlung ist nur für deutsche Bestellungen verfügbar",
  },
  ad: {
    adIdRequired: "Anzeigen-ID ist erforderlich",
    notFound: "Anzeige nicht gefunden",
  },
  favorite: {
    userNotFound: "Benutzer nicht gefunden",
    foodIdRequired: "Speisen-ID ist erforderlich",
    foodAdded: "Speise zu Favoriten hinzugefügt",
    foodRemoved: "Speise aus Favoriten entfernt",
    restaurantIdRequired: "Restaurant-ID ist erforderlich",
    restaurantAdded: "Restaurant zu Favoriten hinzugefügt",
    restaurantRemoved: "Restaurant aus Favoriten entfernt",
  },
  rating: {
    invalidFoodId: "Ungültige Speisen-ID",
    foodNotFound: "Speise nicht gefunden",
    invalidRating: "Bewertung muss zwischen 1 und 5 liegen",
    rated: "Erfolgreich bewertet",
    // v4.2 — aus dem alten toten Block wiederhergestellt
    invalidOrderId: "Ungültige Bestellungs-ID",
    orderNotFound: "Bestellung nicht gefunden oder noch nicht geliefert",
    noDriver: "Kein Fahrer dieser Bestellung zugewiesen",
    driverNotFound: "Fahrer nicht gefunden",
  },
  general: {
    serverError: "Serverfehler",
  },
};

const getMessages = (req) => {
  const header = req?.headers?.["accept-language"] || "en";
  // نأخذ أول لغة فقط (مثلاً "ar-SY,ar;q=0.9" → "ar")
  const lang = header.split(",")[0].split("-")[0].toLowerCase();
  return translations[lang] || translations["en"];
};

/**
 * v2.0 — استخراج رمز اللغة الخام من الطلب (en/ar/de) بدون إرجاع كتلة الترجمات كاملة.
 * مستخدمة لتحديد لغة رسائل SMS عبر Aman Gate (يدعم فقط 0=Arabic / 1=English)،
 * لذلك أي لغة غير "ar" (بما فيها "de") بترجع "en" كافتراضي عند استخدامها لـ SMS.
 */
const getLang = (req) => {
  const header = req?.headers?.["accept-language"] || "en";
  const lang = header.split(",")[0].split("-")[0].toLowerCase();
  return translations[lang] ? lang : "en";
};

/**
 * استخراج اللغة من الـ socket handshake query
 * الأولوية: query.lang → "en" افتراضياً
 */
const getSocketMessages = (socket) => {
  const lang = (socket?.handshake?.query?.lang || "en").toLowerCase();
  return translations[lang] || translations["en"];
};

module.exports = { getMessages, getSocketMessages, getLang };
