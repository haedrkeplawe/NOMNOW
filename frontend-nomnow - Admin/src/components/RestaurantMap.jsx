...
// نسخة الأدمن — بلا react-i18next (مشروع الأدمن ما فيه بنية ترجمة إطلاقاً،
// خلافاً لمشروع المطعم). النصوص هون إنكليزي ثابت بالكود، بنفس القيم
// المستخدمة بملف en.json لمشروع المطعم (settings.mapSearch وأخواتها)
// حتى تبقى تجربة الاستخدام متطابقة بين المشروعين.
//
...
const TEXT = {
  mapSearch: "Search for your location...",
  mapSearching: "Searching...",
  mapLocate: "My current location",
  mapLocateDenied:
    "Location permission denied. Please allow it from browser settings.",
  mapLocateUnsupported: "Your browser does not support geolocation.",
};
...
const RestaurantMapInner = ({ setLocation }) => {
  ...
  // v-gmaps-3 — ويدجت الاقتراحات الفورية أثناء الكتابة (Places API New).
  // استبدلنا حقل الإدخال المخصص + البحث اليدوي بـDebounce بويدجت Google
  // الجاهز (PlaceAutocompleteElement) — نفس تجربة خرائط Google نفسها،
  // اقتراحات تظهر أثناء الكتابة بلا استنى. هاي عنصر HTML أصيل من غوغل
  // (مو مكوّن React)، فلازم ينضاف للـDOM إمبراطورياً بـuseEffect، ونربط
  // معه مستمع حدث "gmp-select" العادي بدل onClick بـJSX.
  useEffect(() => {
    if (!placesLib || !autocompleteContainerRef.current) return;

    const el = new placesLib.PlaceAutocompleteElement({
      // v-gmaps-4 — تحيّز النتائج لسوريا أولاً، بلا استبعاد باقي العالم
      // (includedRegionCodes مو حصر صارم، بس ترتيب أولوية بالنتائج)
  ...
  }, [placesLib, map, setLocation]);

  const handleLocate = () => {
    if (!navigator.geolocation) {
      setLocateError(TEXT.mapLocateUnsupported);
      return;
    }
    setLocating(true);
    setLocateError("");
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocateError(TEXT.mapLocateDenied);
        setLocating(false);
      },
    );
  };
  ...
  return (
    <div className="restaurant-map-wrapper">
      {/* حاوية ويدجت البحث بالاقتراحات الفورية — غوغل بيحقن عنصره هون */}
      <div ref={autocompleteContainerRef} className="restaurant-map-search" />
