...
const ShowDriver = ({
  ...
  // DE — Earnings system state — الألماني فقط
  const [earningsOrders, setEarningsOrders] = useState([]);
  const [earningsSummary, setEarningsSummary] = useState(null);
  const [loadingEarnings, setLoadingEarnings] = useState(false);
  const [settlingEarnings, setSettlingEarnings] = useState(false);
  const [confirmSettleEarnings, setConfirmSettleEarnings] = useState(false);
  const [earningsOrderFilter, setEarningsOrderFilter] = useState("all");
  const [expandedEarningsOrder, setExpandedEarningsOrder] = useState(null);
  ...
  }, [selectedDriver?._id, selectedDriver?.cashCreditLimit]);

  // DE — جلب بيانات أجرة السائق الألماني
  useEffect(() => {
    if (!selectedDriver?._id || selectedDriver.country !== "DE") return;
    setLoadingEarnings(true);
    api
      .get(`/admin/drivers/${selectedDriver._id}/cash-orders`)
      .then((res) => {
        setEarningsOrders(res.data.orders || []);
        setEarningsSummary(res.data.summary || null);
      })
      .catch(() => {
        setEarningsOrders([]);
  ...
  // DE — تسوية أجرة السائق الألماني
  const handleSettleEarnings = () => setConfirmSettleEarnings(true);

  const confirmSettleEarningsAction = async () => {
    setConfirmSettleEarnings(false);
    setSettlingEarnings(true);
    try {
      await api.patch(`/admin/drivers/${selectedDriver._id}/earnings-settle`);
      const ordersRes = await api.get(
        `/admin/drivers/${selectedDriver._id}/cash-orders`,
      );
      setEarningsOrders(ordersRes.data.orders || []);
      setEarningsSummary(ordersRes.data.summary || null);
      toast.success("Earnings settled successfully");
    } catch (err) {
      toast.error(err.response?.data?.message || "Error settling earnings");
    } finally {
      setSettlingEarnings(false);
    }
  };
    ...
    <>
          ...
          <div className="popp showdriver globale-popp">
            <div className="container">
              ...
              <div className="fore">
                ...
                {/* DE — زر إدارة الأجرة للألماني فقط */}
                {!isSyrian && (
                  <button className="cash" onClick={() => setType("earnings")}>
                    Earnings Management
                  </button>
                )}
      ...
      {type !== "show" && (
        <div>
              ...
              <div className="container">
                ...
                <div className="one globale-close">
                  <div className="left">
                    ...
                    <div>
                      ...
                      <p>{selectedDriver.name} — Delivery Fee Earnings</p>
                    </div>
                  </div>
                  <div className="right">
                    <IoMdClose
                ...
                {earningsSummary && (
                  <div className="drv-cash-cards">
                    <div className="drv-cash-card pending">
                      ...
                      <h2>
                        {Number(earningsSummary.totalPending).toLocaleString(
                          "en-US",
                          {
                    ...
                    <div className="drv-cash-card settled">
                      <p>Total Settled</p>
                      <h2>
                        {Number(earningsSummary.totalSettled).toLocaleString(
                          "en-US",
                          {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                ...
                <div className="drv-cash-controls">
                  <button
                    ...
                    disabled={
                      settlingEarnings ||
                      (earningsSummary?.pendingCount ?? 0) === 0
                    }
                  >
                    {settlingEarnings
                      ? "Processing…"
                      : "✓ Confirm Bank Transfer Sent"}
                  </button>
                </div>
                ...
                <div className="drv-cash-orders">
                  <div className="drv-cash-orders-head">
                    <h4>Earnings Orders</h4>
                    <div className="drv-cash-filter-tabs">
                      {["all", "pending", "settled"].map((f) => (
                        <button
                          key={f}
                          className={earningsOrderFilter === f ? "active" : ""}
                          onClick={() => setEarningsOrderFilter(f)}
                        >
                          {f}
                        </button>
                      ))}
                    ...
                    <table className="drv-cash-table">
                      ...
                      <tbody>
                        {earningsOrders
                          .filter(
                            (o) =>
                              earningsOrderFilter === "all" ||
                              o.driverPaymentStatus === earningsOrderFilter,
                          )
                          .map((o) => {
                            const isExpanded = expandedEarningsOrder === o._id;
                            return (
                              <>
                                <tr
                                  key={o._id}
                                          ...
                                          {o.taxBreakdown ? (
                                            <>
                                              <div className="drv-cash-exp-item">
                                                <span>
                                                  Food Tax (
                                                  {o.taxBreakdown.foodTaxRate ??
                                                    7}
                                                  % MwSt)
                                                </span>
                                                <strong>
                                                  {Number(
                                                    o.taxBreakdown.foodTax || 0,
                                                  ).toLocaleString("en-US", {
                                                    minimumFractionDigits: 2,
                                                  })}{" "}
                                                  EUR
          ...
          {/* DE — Confirm: Settle Earnings */}
          {confirmSettleEarnings && (
            <div className="popp globale-approve globale-popp drv-confirm-overlay">
              <div className="container">
                <GiMoneyStack className="icon drv-confirm-icon--settle" />
                <h2>Confirm Earnings Settlement</h2>
                <h3>{selectedDriver.name}</h3>
                <p>
                  Have you already transferred the earnings via bank transfer?
                </p>
                <div className="drv-confirm-info">
                  <p>
                    ...
                    <strong>
                      {Number(
                        earningsSummary?.totalPending ?? 0,
                      ).toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                      })}{" "}
                      EUR
                  ...
                  <p>
                    ✓{" "}
                    <strong>{earningsSummary?.pendingCount ?? 0} orders</strong>{" "}
                    will be marked as <strong>settled</strong>
                  </p>
                  <p>⚠ This action cannot be undone</p>
                </div>
                <div className="buttons">
                  <button onClick={() => setConfirmSettleEarnings(false)}>
                    Cancel
                  </button>
                  <button
                    className="approve drv-confirm-btn--settle"
                    onClick={confirmSettleEarningsAction}
                  >
