...
const RejectModal = ({ settlement, onClose, onConfirm, loading }) => {
  ...
  const currency = settlement.restaurantId?.currency || "SYP";
  return (
        ...
        <div className="set-modal__body">
          <div className="set-modal__info">
            ...
            <div style={{ marginLeft: "auto", textAlign: "right" }}>
              <strong>{fmt(settlement.amount, currency)}</strong>
              <div
                style={{
                  fontSize: 11,
          ...
          <textarea
            ...
            rows={4}
          />
          <p className="set-modal__warn">
            ⚠️ Rejecting will return the full balance to the restaurant's
            available funds.
          </p>
        </div>
...
// ─── Approve Modal ────────────────────────────────────────────
const ApproveModal = ({ settlement, onClose, onConfirm, loading }) => {
  const currency = settlement.restaurantId?.currency || "SYP";
  return (
    <div className="set-overlay" onClick={onClose}>
      <div className="set-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="set-modal__body">
          <div className="set-modal__info set-modal__info--approve">
            <BsShop size={15} />
            <span>{settlement.restaurantId?.name || "Restaurant"}</span>
            <div style={{ marginLeft: "auto", textAlign: "right" }}>
              <strong>{fmt(settlement.amount, currency)}</strong>
          ...
          {settlement.note && (
            <div className="set-modal__note">
              <span>📝 Note from restaurant:</span>
              <p>{settlement.note}</p>
            </div>
          )}
          <p className="set-modal__confirm-text">
            Approving this request will mark{" "}
            <strong>{settlement.ordersCount} orders</strong> as paid out. This
...
const Settlements = () => {
  ...
  const fetchSummary = async () => {
    try {
      ...
      const pList = pendingRes.data.settlements || [];
      const aList = approvedRes.data.settlements || [];
      const rList = rejectedRes.data.settlements || [];

      setSummary({
        pending: pList.length,
        approved: aList.length,
        rejected: rList.length,
        pendingByCurrency: groupByCurrency(pList),
        approvedByCurrency: groupByCurrency(aList),
      });
    } catch {
      /* silent */
    }
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await api.patch(`/admin/settlements/${approveTarget._id}/approve`, {
        resolvedBy: "admin",
      });
      toast.success("Settlement approved — orders marked as paid out");
  ...
  const handleRejectConfirm = async (reason) => {
    try {
      ...
      toast.success("Settlement rejected — balance returned to restaurant");
          ...
          <table className="set-table">
            <thead>
              <tr>
                ...
                <th>Requested At</th>
                <th>Resolved At</th>
                {(filter === "pending" || filter === "all") && <th>Actions</th>}
              </tr>
            </thead>
            <tbody>
              {filtered.map((s, i) => (
                <tr key={s._id}>
                  ...
                  <td>
                    <div className="set-restaurant-cell">
                      ...
                      <div>
                        ...
                        <div className="set-restaurant-cell__country">
                          {s.restaurantId?.currency === "EUR"
                            ? "🇩🇪 Germany"
                            : "🇸🇾 Syria"}
                        </div>
                      </div>
                  ...
                  <td>
                    <div className="set-orders-count">
                      <FiPackage size={12} />
