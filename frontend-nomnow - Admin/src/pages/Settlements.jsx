...
const Settlements = () => {
  ...
  const fetchSettlements = async () => {
    try {
      ...
      const res = await api.get(
        `/admin/settlements?${statusQ}limit=100${
  ...
  const fetchSummary = async () => {
    try {
      const res = await api.get(
        `/admin/settlements/summary${countryParam ? `?${countryParam}` : ""}`,
      );
      const { summary: s } = res.data;
      setSummary({
        pending: s.pending.count,
        approved: s.approved.count,
        rejected: s.rejected.count,
        pendingByCurrency: s.pending.byCurrency,
        approvedByCurrency: s.approved.byCurrency,
      });
    } catch {
      /* silent */
    }
