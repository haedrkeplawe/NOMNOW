...
const Drivers = () => {
  ...
  useEffect(() => {
    ...
    // استقبال تحديث الحالة
    socket.on("driver:statusUpdated", (updatedDriver) => {
      setDrivers((prev) =>
        prev.map((driver) =>
  ...
  useEffect(() => {
    ...
    adminSocket.on("driver:statusUpdated", handleStatusUpdate);

    return () => {
      adminSocket.off("driver:statusUpdated", handleStatusUpdate);
    };
  }, []);
