...
const OrderRow = ({ order, commissionRate, currency }) => {
  const [expanded, setExpanded] = useState(false);
  const netProfit =
    order.orderStatus === "delivered"
      ? order.itemsPrice * (1 - commissionRate / 100)
      : null;
    ...
    <>
      <tr className={`fin-ord-row ${expanded ? "expanded" : ""}`}>
        ...
        <td className="fin-ord-commission" style={{ color: "#e7000b" }}>
          {order.orderStatus === "delivered"
            ? fmt((order.itemsPrice * commissionRate) / 100, currency)
            : "—"}
        </td>
...
const Finance = () => {
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    const matchSettle =
      settleFilter === "all" || o.settlementStatus === settleFilter;
    return matchSearch && matchSettle;
  });
    ...
    <div className="fin-page">
      ...
      <div className="fin-orders-card">
        <div className="fin-orders-head">
          <div>
            <h3>Orders</h3>
            <p>All orders with financial breakdown</p>
          </div>
