...
const Orders = () => {
  ...
  useEffect(() => {
    if (!fetchDone || newOrders.length === 0) return;
    setOrders((prev) => {
      const existingIds = new Set(prev.map((o) => o._id.toString())); // ✅ toString
      const fresh = newOrders.filter((o) => !existingIds.has(o._id.toString())); // ✅ toString
      return [...fresh, ...prev];
    });
  }, [newOrders, fetchDone]);
  ...
  return (
    <div className="orders-page">
      <HeadCreateAndDetails
        ...
        text2={"Real-time delivery tracking and order management"}
      />
